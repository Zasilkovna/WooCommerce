...
/**
 * Tracy extension for PacketeryNette DI.
 */
class TracyExtension extends PacketeryNette\DI\CompilerExtension
{
	/** @var bool */
	private $debugMode;
	...
	public function afterCompile(PacketeryNette\PhpGenerator\ClassType $class)
	{
		...
		if ($this->config->netteMailer && $builder->getByType(PacketeryNette\Mail\IMailer::class)) {
			$initialize->addBody($builder->formatPhp('Tracy\Debugger::getLogger()->mailer = ?;', [
				[new PacketeryNette\DI\Statement(Tracy\Bridges\PacketeryNette\MailSender::class, ['fromEmail' => $this->config->fromEmail]), 'send'],
			]));
		}
