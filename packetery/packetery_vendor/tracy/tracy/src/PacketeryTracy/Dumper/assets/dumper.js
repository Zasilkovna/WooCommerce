...
function toggleChildren(cont, usedIds) {
	...
	cont.querySelectorAll(':scope > .tracy-toggle').forEach((el) => {
		...
		if (!usedIds || (id && usedIds[id])) {
			PacketeryTracy.Toggle.toggle(el, false);
		} else {
			usedIds[id] = true;
			PacketeryTracy.Toggle.toggle(el, true, {usedIds: usedIds});
		}
	});
}
...
PacketeryTracy.Dumper = PacketeryTracy.Dumper || Dumper;

function init() {
	PacketeryTracy.Dumper.init();
}
