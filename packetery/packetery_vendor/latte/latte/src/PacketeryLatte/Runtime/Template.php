...
class Template
{
	...
	private function doRender(string $block = null): bool
	{
		...
		if ($this->referenceType === 'import') {
			if ($this->parentName) {
				throw new PacketeryLatte\RuntimeException('Imported template cannot use {extends} or {layout}, use {import}');
			}
	...
	public function renderBlock(string $name, array $params, $mod = null, $layer = null): void
	{
		...
		if (!$block) {
			$hint = ($t = PacketeryLatte\Helpers::getSuggestion($this->getBlockNames($layer), $name))
				? ", did you mean '$t'?"
				: '.';
			$name = $layer ? "$layer $name" : $name;
			throw new PacketeryLatte\RuntimeException("Cannot include undefined block '$name'$hint");
		}
	...
	public function renderBlockParent(string $name, array $params): void
	{
		...
		if (!$block || ($function = next($block->functions)) === false) {
			throw new PacketeryLatte\RuntimeException("Cannot include undefined parent block '$name'.");
		}
		$function($params);
	...
	protected function addBlock(string $name, string $contentType, array $functions, $layer = null): void
	{
		...
		if ($block->contentType === null) {
			...
			throw new PacketeryLatte\RuntimeException(sprintf(
	...
	protected function call($callable)
	{
		if (!is_callable($callable)) {
			throw new PacketeryLatte\SecurityViolationException('Invalid callable.');
		...
		if (!$allowed) {
			...
			throw new PacketeryLatte\SecurityViolationException("Calling $text() is not allowed.");
		}
		return $callable;
	}
	...
	protected function prop($obj, $prop)
	{
		...
		if (is_string($class) && !$this->policy->isPropertyAllowed($class, (string) $prop)) {
			throw new PacketeryLatte\SecurityViolationException("Access to '$prop' property on a $class object is not allowed.");
		}
		return $obj;
	}
