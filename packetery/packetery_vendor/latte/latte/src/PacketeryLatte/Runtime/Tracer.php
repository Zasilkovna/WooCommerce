...
class Tracer
{
	...
	{
		$e = new PacketeryLatte\RuntimeException('Your location in PacketeryLatte templates');
	...
	private static function generateTrace(array $trace): array
	{
		...
		foreach ($trace as $i => $item) {
			...
			if ($object instanceof Template) {
				...
				if (PacketeryLatte\Helpers::startsWith($method, 'block')) {
					// begin of block
	...
	private static function filterParams(array $params): array
	{
		foreach ($params as $key => $foo) {
			if (is_string($key) && PacketeryLatte\Helpers::startsWith($key, 'ʟ_')) {
				unset($params[$key]);
			}
		}
