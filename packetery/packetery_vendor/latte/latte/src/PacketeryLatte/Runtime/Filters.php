...
use PacketeryLatte\RuntimeException;
...
class Filters
{
	...
	public static function escapeJs($s): string
	{
		...
		if ($error = json_last_error()) {
			throw new PacketeryLatte\RuntimeException(json_last_error_msg(), $error);
		}
	...
	public static function indent(FilterInfo $info, string $s, int $level = 1, string $chars = "\t"): string
	{
		if ($level < 1) {
			...
			if (preg_last_error()) {
				throw new PacketeryLatte\RegexpException(null, preg_last_error());
			}
	...
	public static function checkTagSwitch(string $orig, $new): void
	{
		if (
			!is_string($new)
			|| !preg_match('~' . PacketeryLatte\Parser::RE_TAG_NAME . '$~DA', $new)
		) {
			throw new PacketeryLatte\RuntimeException('Invalid tag name ' . var_export($new, true));
		}

		$new = strtolower($new);
		if (
			$new === 'style' || $new === 'script'
			|| isset(PacketeryLatte\Helpers::$emptyElements[strtolower($orig)]) !== isset(PacketeryLatte\Helpers::$emptyElements[$new])
		) {
			throw new PacketeryLatte\RuntimeException("Forbidden tag <$orig> change to <$new>.");
		}
	}
}
