 ...
 * Template loader.
 */
class FileLoader implements PacketeryLatte\Loader
{
	...
	public function getContent($fileName): string
	{
		...
		if ($this->baseDir && !PacketeryLatte\Helpers::startsWith($this->normalizePath($file), $this->baseDir)) {
			throw new PacketeryLatte\RuntimeException("Template '$file' is not within the allowed path '{$this->baseDir}'.");

		} elseif (!is_file($file)) {
			throw new PacketeryLatte\RuntimeException("Missing template file '$file'.");
