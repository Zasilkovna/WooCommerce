...
namespace PacketeryLatte\Macros;

use PacketeryLatte;
use PacketeryLatte\CompileException;
use PacketeryLatte\Engine;
use PacketeryLatte\Helpers;
use PacketeryLatte\MacroNode;
use PacketeryLatte\PhpHelpers;
use PacketeryLatte\PhpWriter;


/**
 * Basic macros for PacketeryLatte.
 */
class CoreMacros extends MacroSet
{
	...
	public static function install(PacketeryLatte\Compiler $compiler): void
	{
	...
	public function finalize()
	{
		if ($this->printTemplate) {
			return ["(new PacketeryLatte\\Runtime\\Blueprint)->printClass(\$this, {$this->printTemplate}); exit;"];
		}

		$code = '';
		if ($this->overwrittenVars) {
			$vars = array_map(function ($l) { return implode(', ', $l); }, $this->overwrittenVars);
			$code .= 'foreach (array_intersect_key(' . PacketeryLatte\PhpHelpers::dump($vars) . ', $this->params) as $ʟ_v => $ʟ_l) { '
	...
	public function macroSpaceless(MacroNode $node, PhpWriter $writer): void
	{
		...
		$node->openingCode = $writer->write(in_array($node->context[0], [Engine::CONTENT_HTML, Engine::CONTENT_XHTML], true)
			? "<?php ob_start('PacketeryLatte\\Runtime\\Filters::spacelessHtmlHandler', 4096) %node.line; ?>"
			: "<?php ob_start('PacketeryLatte\\Runtime\\Filters::spacelessText', 4096) %node.line; ?>");
	...
	public function macroTagEnd(MacroNode $node, PhpWriter $writer): void
	{
		...
		$node->openingCode = $writer->write('<?php
			...
			PacketeryLatte\Runtime\Filters::checkTagSwitch(%1_var, $ʟ_tag[%0_var]);
		?>', $node->htmlNode->data->id, $node->htmlNode->name);

		$node->content = preg_replace(
			'~^(\s*<)' . PacketeryLatte\Parser::RE_TAG_NAME . '~',
			"\$1<?php echo \$ʟ_tag[{$node->htmlNode->data->id}]; ?>\n",
			$node->content
		);
		$node->content = preg_replace(
			'~</' . PacketeryLatte\Parser::RE_TAG_NAME . '(\s*>\s*)$~',
	...
	public function macroVar(MacroNode $node, PhpWriter $writer): string
	{
		...
		$res = new PacketeryLatte\MacroTokens;
	...
	public function macroVarPrint(MacroNode $node): string
	{
		...
		return "(new PacketeryLatte\\Runtime\\Blueprint)->printVars($vars); exit;";
	}
