 ...
 */
class MacroSet implements PacketeryLatte\Macro
{
	use PacketeryLatte\Strict;

	/** @var PacketeryLatte\Compiler */
	private $compiler;
	...
	public function __construct(PacketeryLatte\Compiler $compiler)
	{
	...
	public function addMacro(string $name, $begin, $end = null, $attr = null, int $flags = null): self
	{
		...
		foreach ([$begin, $end, $attr] as $arg) {
			if ($arg && !is_string($arg)) {
				PacketeryLatte\Helpers::checkCallback($arg);
			}
		}
	...
	public function nodeOpened(MacroNode $node)
	{
		...
		if ($attr && $node->prefix === $node::PREFIX_NONE) {
			$node->empty = true;
			$node->context[1] = PacketeryLatte\Compiler::CONTEXT_HTML_ATTRIBUTE;
			...
			$node->context[1] = PacketeryLatte\Compiler::CONTEXT_HTML_TEXT;
	...
	private function compile(MacroNode $node, $def)
	{
		...
		$writer = PacketeryLatte\PhpWriter::using($node, $this->compiler);
	...
	public function getCompiler(): PacketeryLatte\Compiler
	{
	...
	protected function checkExtraArgs(MacroNode $node): void
	{
		if ($node->tokenizer->isNext()) {
			$args = PacketeryLatte\Runtime\Filters::truncate($node->tokenizer->joinAll(), 20);
