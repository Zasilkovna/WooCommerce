...
namespace PacketeryLatte\Bridges\PacketeryTracy;
...
/**
 * BlueScreen panels for PacketeryTracy 2.x
 */
class BlueScreenPanel
{
	public static function initialize(BlueScreen $blueScreen = null): void
	{
		$blueScreen = $blueScreen ?? PacketeryTracy\Debugger::getBlueScreen();
	...
	{
		if ($e instanceof PacketeryLatte\CompileException && $e->sourceName) {
			return [
				'tab' => 'Template',
				'panel' => (preg_match('#\n|\?#', $e->sourceName)
						? ''
		...
		} elseif ($e && strpos($file = $e->getFile(), '.view--')) {
			$lines = file($file);
			if (
				preg_match('#/\*\* source: (\S+\.view)#', $lines[4], $m)
	...
	{
		if (
			$e instanceof PacketeryLatte\CompileException
			&& $e->sourceName
			&& @is_file($e->sourceName) // @ - may trigger error
