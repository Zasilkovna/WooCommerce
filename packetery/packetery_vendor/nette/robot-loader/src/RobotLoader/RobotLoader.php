...
namespace PacketeryNette\Loaders;
...
/**
 * PacketeryNette auto loader is responsible for loading classes and interfaces.
 *
 * <code>
...
class RobotLoader
{
	...
	public function __construct()
	{
		if (!extension_loaded('tokenizer')) {
			throw new PacketeryNette\NotSupportedException('PHP extension Tokenizer is not loaded.');
		}
	}
			...
			foreach ($iterator as $fileInfo) {
				...
				foreach ($foundClasses as $class) {
					if (isset($this->classes[$class])) {
						throw new PacketeryNette\InvalidStateException("Ambiguous class $class resolution; defined in {$this->classes[$class][0]} and in $file.");
					}
	 ...
	 * @throws PacketeryNette\IOException if path is not found
	 */
	private function createFileIterator(string $dir): PacketeryNette\Utils\Finder
	{
		if (!is_dir($dir)) {
			throw new PacketeryNette\IOException("File or directory '$dir' not found.");
		}
		...
		$iterator = PacketeryNette\Utils\Finder::findFiles($acceptFiles)
	...
	private function updateFile(string $file): void
	{
		...
		foreach ($foundClasses as $class) {
			...
			if (isset($prevFile)) {
				throw new PacketeryNette\InvalidStateException("Ambiguous class $class resolution; defined in $prevFile and in $file.");
			}
