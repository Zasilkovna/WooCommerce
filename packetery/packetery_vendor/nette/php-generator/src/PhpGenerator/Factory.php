...
final class Factory
{
	...
	public function fromClassReflection(\ReflectionClass $from, bool $withBodies = false): ClassType
	{
		...
		foreach ($from->getMethods() as $method) {
			if ($method->getDeclaringClass()->name === $from->name) {
				...
				if ($withBodies) {
					$srcMethod = PacketeryNette\Utils\Reflection::getMethodDeclaringMethod($method);
	...
	{
		$ref = PacketeryNette\Utils\Callback::toReflection($from);
	...
	{
		if ($from->isAnonymous()) {
			throw new PacketeryNette\NotSupportedException('Anonymous classes are not supported.');
		}
	...
	{
		if ($from->isClosure()) {
			throw new PacketeryNette\NotSupportedException('Closures are not supported.');
		}
	...
	private function parse($from): array
	{
		...
		if (!class_exists(ParserFactory::class)) {
			throw new PacketeryNette\NotSupportedException("PHP-Parser is required to load method bodies, install package 'nikic/php-parser'.");
		} elseif (!$file) {
			throw new PacketeryNette\InvalidStateException("Source code of $from->name not found.");
		}
