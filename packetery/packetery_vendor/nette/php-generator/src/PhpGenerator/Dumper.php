...
final class Dumper
{
	...
	private function dumpVar(&$var, array $parents = [], int $level = 0, int $column = 0): string
	{
		if ($var === null) {
			...
			if ($var instanceof Literal || $var instanceof Closure) {
				return ltrim(PacketeryNette\Utils\Strings::indent(trim((string) $var), $level), "\t");
			}
			return $this->dumpObject($var, $parents, $level);

		} elseif (is_resource($var)) {
			throw new PacketeryNette\InvalidArgumentException('Cannot dump resource.');
	...
	private function dumpArray(array &$var, array $parents, int $level, int $column): string
	{
		if (empty($var)) {
			...
			throw new PacketeryNette\InvalidArgumentException('Nesting level too deep or recursive dependency.');
		}
	...
	private function dumpObject(&$var, array $parents, int $level): string
	{
		if ($var instanceof \Serializable) {
			...
			throw new PacketeryNette\InvalidArgumentException('Cannot dump closure.');
		}

		$class = get_class($var);
		if ((new \ReflectionObject($var))->isAnonymous()) {
			throw new PacketeryNette\InvalidArgumentException('Cannot dump anonymous class.');
	...
	public function format(string $statement, ...$args): string
	{
		...
		foreach ($tokens as $n => $token) {
			if ($n % 2 === 0) {
				...
				throw new PacketeryNette\InvalidArgumentException('Insufficient number of arguments.');
			} elseif ($token === '?') {
				$res .= $this->dump(array_shift($args), strlen($res) - strrpos($res, "\n"));
			} elseif ($token === '...?' || $token === '...?:' || $token === '?*') {
				$arg = array_shift($args);
				if (!is_array($arg)) {
					throw new PacketeryNette\InvalidArgumentException('Argument must be an array.');
				}
		...
		if ($args) {
			throw new PacketeryNette\InvalidArgumentException('Insufficient number of placeholders.');
		}
		return $res;
	}
