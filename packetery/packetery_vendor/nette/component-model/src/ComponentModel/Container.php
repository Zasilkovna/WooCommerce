...
class Container extends Component implements IContainer
{
	...
	public function addComponent(IComponent $component, ?string $name, string $insertBefore = null)
	{
		if ($name === null) {
			...
			if ($name === null) {
				throw new PacketeryNette\InvalidStateException("Missing component's name.");
			}
		}

		if (!preg_match(self::NAME_REGEXP, $name)) {
			throw new PacketeryNette\InvalidArgumentException("Component name must be non-empty alphanumeric string, '$name' given.");
		}

		if (isset($this->components[$name])) {
			throw new PacketeryNette\InvalidStateException("Component with name '$name' already exists.");
		}
		...
		do {
			if ($obj === $component) {
				throw new PacketeryNette\InvalidStateException("Circular reference detected while adding component '$name'.");
			}
			$obj = $obj->getParent();
		} while ($obj !== null);
	...
	public function removeComponent(IComponent $component): void
	{
		...
		if (($this->components[$name] ?? null) !== $component) {
			throw new PacketeryNette\InvalidArgumentException("Component named '$name' is not located in this container.");
		}
	...
	final public function getComponent(string $name, bool $throw = true): ?IComponent
	{
		...
		if ($component !== null) {
			if (!isset($parts[1])) {
				...
				throw new PacketeryNette\InvalidArgumentException("Component with name '$name' is not container and cannot have '$parts[1]' component.");
			}

		} elseif ($throw) {
			$hint = PacketeryNette\Utils\ObjectHelpers::getSuggestion(array_merge(
				array_map('strval', array_keys($this->components)),
				array_map('lcfirst', preg_filter('#^createComponent([A-Z0-9].*)#', '$1', get_class_methods($this)))
			), $name);
			throw new PacketeryNette\InvalidArgumentException("Component with name '$name' does not exist" . ($hint ? ", did you mean '$hint'?" : '.'));
		}
		return null;
	}
	...
	protected function createComponent(string $name): ?IComponent
	{
		...
		if ($ucname !== $name && method_exists($this, $method) && (new \ReflectionMethod($this, $method))->getName() === $method) {
			...
			if (!$component instanceof IComponent && !isset($this->components[$name])) {
				...
				throw new PacketeryNette\UnexpectedValueException("Method $class::$method() did not return or create the desired component.");
			}
			return $component;
		}
		return null;
	...
	/**
	 * Descendant can override this method to disallow insert a child by throwing an PacketeryNette\InvalidStateException.
	 * @throws PacketeryNette\InvalidStateException
	 */
