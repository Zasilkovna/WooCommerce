...
abstract class CompilerExtension
{
	...
	/** @var PacketeryNette\PhpGenerator\Closure */
	...
	{
		$this->initialization = new PacketeryNette\PhpGenerator\Closure;
	...
	public function getConfigSchema(): PacketeryNette\Schema\Schema
	{
		return is_object($this->config)
			? PacketeryNette\Schema\Expect::from($this->config)
			: PacketeryNette\Schema\Expect::array();
	}
	...
	public function validateConfig(array $expected, array $config = null, string $name = null): array
	{
		...
		if ($extra = array_diff_key((array) $config, $expected)) {
			...
			$hint = PacketeryNette\Utils\Helpers::getSuggestion(array_keys($expected), key($extra));
			$extra = $hint
				? key($extra)
				: implode("', '{$name} › ", array_keys($extra));
			throw new PacketeryNette\DI\InvalidConfigurationException("Unknown configuration option '{$name} › {$extra}'" . ($hint ? ", did you mean '{$name} › {$hint}'?" : '.'));
		}
		return PacketeryNette\Schema\Helpers::merge($config, $expected);
	}
	...
	public function getInitialization(): PacketeryNette\PhpGenerator\Closure
	{
