...
class DefinitionSchema implements Schema
{
	...
	/** @var PacketeryNette\DI\ContainerBuilder */
	private $builder;


	public function __construct(PacketeryNette\DI\ContainerBuilder $builder)
	{
	...
	public function merge($def, $base)
	{
		...
		return PacketeryNette\Schema\Helpers::merge($def, $base);
	}
	...
	public function normalize($def, Context $context)
	{
		if ($def === null || $def === false) {
			...
			foreach (['class' => 'type', 'dynamic' => 'imported'] as $alias => $original) {
				if (array_key_exists($alias, $def)) {
					if (array_key_exists($original, $def)) {
						throw new PacketeryNette\DI\InvalidConfigurationException("Options '$alias' and '$original' are aliases, use only '$original'.");
					}
			...
			throw new PacketeryNette\DI\InvalidConfigurationException('Unexpected format of service definition');
		}
	}
	...
	private function expandParameters(array $config): array
	{
		...
		return PacketeryNette\DI\Helpers::expand($config, $params);
	}
	...
	private static function getServiceSchema(): Schema
	{
		return Expect::structure([
			...
			'factory' => Expect::type('callable|PacketeryNette\DI\Definitions\Statement'),
			'arguments' => Expect::array(),
			'setup' => Expect::listOf('callable|PacketeryNette\DI\Definitions\Statement|array:1'),
			'inject' => Expect::bool(),
