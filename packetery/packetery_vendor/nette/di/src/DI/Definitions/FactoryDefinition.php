...
use PacketeryNette\Utils\Reflection;
...
final class FactoryDefinition extends Definition
{
	...
	public function setImplement(string $type)
	{
		if (!interface_exists($type)) {
			throw new PacketeryNette\InvalidArgumentException("Service '{$this->getName()}': Interface '$type' not found.");
		}
		$rc = new \ReflectionClass($type);
		$method = $rc->getMethods()[0] ?? null;
		if (!$method || $method->isStatic() || $method->name !== self::METHOD_CREATE || count($rc->getMethods()) > 1) {
			throw new PacketeryNette\InvalidArgumentException("Service '{$this->getName()}': Interface $type must have just one non-static method create().");
		}
	...
	public function resolveType(PacketeryNette\DI\Resolver $resolver): void
	{
		...
		if (!$resultDef->getType()) {
			...
			$returnType = PacketeryNette\DI\Helpers::getReturnType($method);
			if (!$returnType) {
		...
		if ($resultDef instanceof ServiceDefinition) {
			...
			if ($resultDef->getEntity() instanceof Reference && !$resultDef->getFactory()->arguments) {
				$resultDef->setFactory([ // render as $container->createMethod()
					new Reference(PacketeryNette\DI\ContainerBuilder::THIS_CONTAINER),
					PacketeryNette\DI\Container::getMethodName($resultDef->getEntity()->getValue()),
				]);
			}
		}
	...
	private function completeParameters(PacketeryNette\DI\Resolver $resolver): void
	{
		$interface = $this->getType();
		$method = new \ReflectionMethod($interface, self::METHOD_CREATE);
		...
		foreach ($method->getParameters() as $param) {
			...
			if (isset($ctorParams[$param->name])) {
				...
				$this->resultDefinition->getFactory()->arguments[$ctorParam->getPosition()] = PacketeryNette\DI\ContainerBuilder::literal('$' . $ctorParam->name);

			} elseif (!$this->resultDefinition->getSetup()) {
				$hint = PacketeryNette\Utils\Helpers::getSuggestion(array_keys($ctorParams), $param->name);
	...
	{
		$class = (new PacketeryNette\PhpGenerator\ClassType)
