...
class Compiler
{
	...
	public function addExtension(?string $name, CompilerExtension $extension)
	{
		if ($name === null) {
			...
			throw new PacketeryNette\InvalidArgumentException("Name '$name' is already used or reserved.");
		}
		$lname = strtolower($name);
		foreach (array_keys($this->extensions) as $nm) {
			if ($lname === strtolower((string) $nm)) {
				throw new PacketeryNette\InvalidArgumentException("Name of extension '$name' has the same name as '$nm' in a case-insensitive manner.");
			}
		}
	...
	public function processExtensions(): void
	{
		...
		if ($decorator = $this->getExtensions(Extensions\DecoratorExtension::class)) {
			PacketeryNette\Utils\Arrays::insertBefore($this->extensions, key($decorator), $this->getExtensions(Extensions\SearchExtension::class));
		}
		...
		if ($extra = array_diff_key($this->extensions, $extensions, $first, [self::SERVICES => 1])) {
			...
			throw new PacketeryNette\DeprecatedException("Extensions '$extra' were added while container was being compiled.");

		} elseif ($extra = key(array_diff_key($this->configs, $this->extensions))) {
			$hint = PacketeryNette\Utils\Helpers::getSuggestion(array_keys($this->extensions), $extra);
	...
	private function processSchema(Schema\Schema $schema, array $configs, $name = null)
	{
		...
		try {
			...
			throw new PacketeryNette\DI\InvalidConfigurationException($e->getMessage());
		}
	...
	{
		throw new PacketeryNette\DeprecatedException(__METHOD__ . '() is deprecated, use non-static Compiler::loadDefinitionsFromConfig(array $configList).');
	}
