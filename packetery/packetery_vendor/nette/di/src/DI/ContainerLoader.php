...
class ContainerLoader
{
	...
	/**
	 * @param  callable  $generator  function (PacketeryNette\DI\Compiler $compiler): string|null
	 * @param  mixed  $key
	 */
	...
	private function loadFile(string $class, callable $generator): void
	{
		...
		PacketeryNette\Utils\FileSystem::createDir($this->tempDirectory);

		$handle = @fopen("$file.lock", 'c+'); // @ is escalated to exception
		if (!$handle) {
			throw new PacketeryNette\IOException("Unable to create file '$file.lock'. " . PacketeryNette\Utils\Helpers::getLastError());
		} elseif (!@flock($handle, LOCK_EX)) { // @ is escalated to exception
			throw new PacketeryNette\IOException("Unable to acquire exclusive lock on '$file.lock'. " . PacketeryNette\Utils\Helpers::getLastError());
		}
		...
		if (!is_file($file) || $this->isExpired($file, $updatedMeta)) {
			...
			foreach ($toWrite as $name => $content) {
				if (file_put_contents("$name.tmp", $content) !== strlen($content) || !rename("$name.tmp", $name)) {
					...
					throw new PacketeryNette\IOException("Unable to create file '$name'.");
		...
		if ((@include $file) === false) { // @ - error escalated to exception
			throw new PacketeryNette\IOException("Unable to include '$file'.");
		}
