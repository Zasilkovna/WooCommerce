...
use PacketeryNette\Utils\Validators;
use ReflectionClass;
	...
	public function resolveEntityType(Statement $statement): ?string
	{
		...
		if (is_array($entity)) {
			...
			try {
				...
				$reflection = PacketeryNette\Utils\Callback::toReflection($entity[0] === '' ? $entity[1] : $entity);
			...
			if (isset($e) || ($refClass && (!$reflection->isPublic()
				...
				throw new ServiceCreationException(sprintf('Method %s() is not callable.', PacketeryNette\Utils\Callback::toString($entity)), 0, $e ?? null);
			}
			$this->addDependency($reflection);

			$type = Helpers::getReturnType($reflection);
			if ($type && !class_exists($type) && !interface_exists($type)) {
				throw new ServiceCreationException(sprintf("Class or interface '%s' not found. Is return type of %s() correct?", $type, PacketeryNette\Utils\Callback::toString($entity)));
			}
			return $type;
			...
			case is_array($entity):
				...
				switch (true) {
					case $entity[0] === '': // function call
						if (!PacketeryNette\Utils\Arrays::isList($arguments)) {
					...
					case $entity[0] instanceof Reference:
						if ($entity[1][0] === '$') { // property getter, setter or appender
							Validators::assert($arguments, 'list:0..1', "setup arguments for '" . PacketeryNette\Utils\Callback::toString($entity) . "'");
							...
							} elseif (!PacketeryNette\Utils\Arrays::isList($arguments)) {
