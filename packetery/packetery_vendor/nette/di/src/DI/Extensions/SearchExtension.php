...
use PacketeryNette;
use PacketeryNette\Loaders\RobotLoader;
 ...
 */
final class SearchExtension extends PacketeryNette\DI\CompilerExtension
{
	/** @var array */
	private $classes = [];
	...
	public function loadConfiguration()
	{
		foreach (array_filter($this->config) as $name => $batch) {
			if (!is_dir($batch->in)) {
				throw new PacketeryNette\DI\InvalidConfigurationException("Option '{$this->name} › {$name} › in' must be valid directory name, '{$batch->in}' given.");
			}
	...
	public function findClasses(\stdClass $config): array
	{
		...
		foreach ($classes as $class) {
			if (!class_exists($class) && !interface_exists($class) && !trait_exists($class)) {
				throw new PacketeryNette\InvalidStateException("Class $class was found, but it cannot be loaded by autoloading.");
			}
