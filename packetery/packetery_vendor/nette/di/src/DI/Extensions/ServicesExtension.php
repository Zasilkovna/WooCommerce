...
use PacketeryNette\DI\Helpers;


/**
 * Service definitions loader.
 */
final class ServicesExtension extends PacketeryNette\DI\CompilerExtension
{
	use PacketeryNette\SmartObject;

	public function getConfigSchema(): PacketeryNette\Schema\Schema
	{
		return PacketeryNette\Schema\Expect::arrayOf(new PacketeryNette\DI\Config\DefinitionSchema($this->getContainerBuilder()));
	}
	...
	private function loadDefinition(?string $name, \stdClass $config): void
	{
		try {
			if ((array) $config === [false]) {
				...
				throw new PacketeryNette\DI\InvalidConfigurationException('missing original definition for alteration.');
			}
			...
			throw new PacketeryNette\DI\InvalidConfigurationException(($name ? "Service '$name': " : '') . $e->getMessage(), 0, $e);
		}
	}
	...
	private function updateServiceDefinition(Definitions\ServiceDefinition $definition, \stdClass $config): void
	{
		...
		if ($config->arguments) {
			...
			if (empty($config->reset['arguments']) && !PacketeryNette\Utils\Arrays::isList($arguments)) {
