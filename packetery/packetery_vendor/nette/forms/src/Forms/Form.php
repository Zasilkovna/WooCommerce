...
use PacketeryNette;
use PacketeryNette\Utils\Arrays;
 ...
 */
class Form extends Container implements PacketeryNette\HtmlStringable
{
	/** validator */
	public const
		EQUAL = ':equal',
	...
	/** @internal @var PacketeryNette\Http\IRequest  used only by standalone form */
	public $httpRequest;
	...
	/** @var PacketeryNette\Localization\Translator */
	private $translator;
	...
	public function __construct(string $name = null)
	{
		...
		$this->monitor(self::class, function (): void {
			throw new PacketeryNette\InvalidStateException('Nested forms are forbidden.');
		});
	}
	...
	public function setMethod(string $method)
	{
		if ($this->httpData !== null) {
			throw new PacketeryNette\InvalidStateException(__METHOD__ . '() must be called until the form is empty.');
		}
	...
	public function removeGroup($name): void
	{
		if (is_string($name) && isset($this->groups[$name])) {
			...
			throw new PacketeryNette\InvalidArgumentException("Group not found in form '$this->name'");
		}
	 ...
	 * @return static
	 */
	public function setTranslator(?PacketeryNette\Localization\Translator $translator)
	{
	 ...
	 */
	public function getTranslator(): ?PacketeryNette\Localization\Translator
	{
	...
	public function getHttpData(int $type = null, string $htmlName = null)
	{
		if ($this->httpData === null) {
			if (!$this->isAnchored()) {
				throw new PacketeryNette\InvalidStateException('Form is not anchored and therefore can not determine whether it was submitted.');
			}
	...
	private function invokeHandlers(iterable $handlers, $button = null): void
	{
		foreach ($handlers as $handler) {
			$params = PacketeryNette\Utils\Callback::toReflection($handler)->getParameters();
			$types = array_map([PacketeryNette\Utils\Reflection::class, 'getParameterType'], $params);
			if (!isset($types[0])) {
				$arg0 = $button ?: $this;
	...
	protected function receiveHttpData(): ?array
	{
		...
		if ($httpRequest->isMethod('post')) {
			...
			$data = PacketeryNette\Utils\Arrays::mergeTree($httpRequest->getPost(), $httpRequest->getFiles());
		} else {
	...
	public static function initialize(bool $reinit = false): void
	{
		...
		self::$defaultHttpRequest = (new PacketeryNette\Http\RequestFactory)->fromGlobals();

		if (PHP_SAPI !== 'cli') {
			if (headers_sent($file, $line)) {
				throw new PacketeryNette\InvalidStateException(
					'Create a form or call PacketeryNette\Forms\Form::initialize() before the headers are sent to initialize CSRF protection.'
					. ($file ? " (output started at $file:$line)" : '') . '. '
				);
			}
			PacketeryNette\Http\Helpers::initCookie(self::$defaultHttpRequest, new PacketeryNette\Http\Response);
		}
	}


	/** @internal */
	public function setHttpRequest(PacketeryNette\Http\IRequest $request)
	{
		$this->httpRequest = $request;
	}


	private function getHttpRequest(): PacketeryNette\Http\IRequest
	{
		if (!$this->httpRequest) {
			self::initialize();
