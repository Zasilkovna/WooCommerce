...
class Helpers
{
	...
	public static function extractHttpData(array $data, string $htmlName, int $type)
	{
		...
		$data = PacketeryNette\Utils\Arrays::get($data, $name, null);
	...
	private static function sanitize(int $type, $value)
	{
		if ($type === Form::DATA_TEXT) {
			...
			return $value instanceof PacketeryNette\Http\FileUpload ? $value : null;

		} else {
			throw new PacketeryNette\InvalidArgumentException('Unknown data type');
		}
	}
	...
	{
		$name = str_replace(PacketeryNette\ComponentModel\IComponent::NAME_SEPARATOR, '][', $id, $count);
		if ($count) {
	...
	public static function exportRules(Rules $rules): array
	{
		...
		foreach ($rules as $rule) {
			...
			$op = $rule->validator;
			if (!is_string($op)) {
				$op = PacketeryNette\Utils\Callback::toString($op);
			}
			...
			if ($rule->branch) {
				...
				if ($msg instanceof PacketeryNette\HtmlStringable) {
	...
	public static function createInputList(
		...
		foreach ($items as $value => $caption) {
			...
			$res .= ($res === '' && $wrapperEnd === '' ? '' : $wrapper)
				...
				. ($caption instanceof PacketeryNette\HtmlStringable ? $caption : htmlspecialchars((string) $caption, ENT_NOQUOTES, 'UTF-8'))
				. '</label>'
				. $wrapperEnd;
		}
		return $res;
