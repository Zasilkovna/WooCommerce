...
namespace PacketeryNette\Bridges\FormsPacketeryLatte;
...
/**
 * PacketeryLatte macros for PacketeryNette\Forms.
 *
 * - {form name} ... {/form}
 * - {input name}
...
final class FormMacros extends MacroSet
{
	public static function install(PacketeryLatte\Compiler $compiler): void
	{
		...
		$me->addMacro('form', [$me, 'macroForm'], 'echo PacketeryNette\Bridges\FormsPacketeryLatte\Runtime::renderFormEnd(array_pop($this->global->formsStack));');
	...
	public function macroForm(MacroNode $node, PhpWriter $writer)
	{
		...
		return $writer->write(
			'echo PacketeryNette\Bridges\FormsPacketeryLatte\Runtime::renderFormBegin($form = $this->global->formsStack[] = '
	...
	public function macroNameAttr(MacroNode $node, PhpWriter $writer)
	{
		...
		if ($tagName === 'form') {
			...
			return $writer->write(
				'echo PacketeryNette\Bridges\FormsPacketeryLatte\Runtime::renderFormBegin(end($this->global->formsStack), %0.var, false)',
	...
	public function macroNameEnd(MacroNode $node, PhpWriter $writer)
	{
		$tagName = strtolower($node->htmlNode->name);
		if ($tagName === 'form') {
			$node->innerContent .= '<?php echo PacketeryNette\Bridges\FormsPacketeryLatte\Runtime::renderFormEnd(array_pop($this->global->formsStack), false)'
				. " /* line $node->startLine */; ?>";
	...
	public function macroFormPrint(MacroNode $node, PhpWriter $writer)
	{
		...
		$node->tokenizer->reset();
		return $writer->write(
			'PacketeryNette\Bridges\FormsPacketeryLatte\Runtime::renderBlueprint('
			. ($name[0] === '$' ? 'is_object(%node.word) ? %node.word : ' : '')
