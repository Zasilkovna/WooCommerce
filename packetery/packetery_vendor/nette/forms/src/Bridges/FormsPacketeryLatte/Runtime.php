...
/**
 * Runtime helpers for PacketeryLatte.
 * @internal
 */
class Runtime
{
	...
	public static function renderFormEnd(Form $form, bool $withTags = true): string
	{
		...
		if (iterator_count($form->getComponents(true, PacketeryNette\Forms\Controls\TextInput::class)) < 2) {
	...
	public static function renderBlueprint($form): void
	{
		...
		foreach ($form->getControls() as $name => $input) {
			$dict[$input] = $dummyInput = new class extends PacketeryNette\Forms\Controls\BaseControl {
				public $inner;
		...
		if ($renderer instanceof PacketeryNette\Forms\Rendering\DefaultFormRenderer) {
		...
		$blueprint = new PacketeryLatte\Runtime\Blueprint;
