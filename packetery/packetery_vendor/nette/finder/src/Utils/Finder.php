...
class Finder implements \IteratorAggregate, \Countable
{
	...
	{
		if ($this->paths) {
			throw new PacketeryNette\InvalidStateException('Directory to search has already been specified.');
		}
	...
	{
		if (!$this->paths) {
			throw new PacketeryNette\InvalidStateException('Call in() or from() to specify directory to search.');
	...
	public function size(string $operator, int $size = null): self
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?((?:\d*\.)?\d+)\s*(K|M|G|)B?$#Di', $operator, $matches)) {
				throw new PacketeryNette\InvalidArgumentException('Invalid size predicate format.');
			}
	...
	public function date(string $operator, $date = null): self
	{
		if (func_num_args() === 1) { // in $operator is predicate
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?(.+)$#Di', $operator, $matches)) {
				throw new PacketeryNette\InvalidArgumentException('Invalid date predicate format.');
			}
	...
	public static function compare($l, string $operator, $r): bool
	{
		switch ($operator) {
			...
			case '<>':
				return $l != $r;
			default:
				throw new PacketeryNette\InvalidArgumentException("Unknown operator $operator.");
		}
	}
	...
	public function __call(string $name, array $args)
	{
		return isset(self::$extMethods[$name])
			...
			: PacketeryNette\Utils\ObjectHelpers::strictCall(get_class($this), $name, array_keys(self::$extMethods));
	}
