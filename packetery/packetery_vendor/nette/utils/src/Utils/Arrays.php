...
class Arrays
{
	 ...
	 * @param  mixed  $default
	 * @return mixed
	 * @throws PacketeryNette\InvalidArgumentException if item does not exist and default value is not provided
	 */
	public static function get(array $array, $key, $default = null)
	{
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) && array_key_exists($k, $array)) {
				...
				if (func_num_args() < 3) {
					throw new PacketeryNette\InvalidArgumentException("Missing item '$k'.");
				}
				return $default;
			}
		}
	 ...
	 * @param  string|int|array  $key one or more keys
	 * @return mixed
	 * @throws PacketeryNette\InvalidArgumentException if traversed item is not an array
	 */
	public static function &getRef(array &$array, $key)
	{
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) || $array === null) {
				...
				throw new PacketeryNette\InvalidArgumentException('Traversed item is not an array.');
			}
		}
		return $array;
	}
	 ...
	 * @throws PacketeryNette\RegexpException  on compilation or runtime error
	 */
	...
	public static function associate(array $array, $path)
	{
		...
		if (!$parts || $parts === ['->'] || $parts[0] === '=' || $parts[0] === '|') {
			throw new PacketeryNette\InvalidArgumentException("Invalid path '$path'.");
		}
	...
	public static function pick(array &$array, $key, $default = null)
	{
		if (array_key_exists($key, $array)) {
			...
			throw new PacketeryNette\InvalidArgumentException("Missing item '$key'.");
