...
trait Base
{
	...
	private function doDeprecation(Context $context): void
	{
		if ($this->deprecated !== null) {
			$context->addWarning(
				$this->deprecated,
				PacketeryNette\Schema\Message::DEPRECATED
			);
		}
	}
	...
	{
		if (!PacketeryNette\Utils\Validators::is($value, $expected)) {
	...
	private function doValidateRange($value, array $range, Context $context, string $types = ''): bool
	{
		if (is_array($value) || is_string($value)) {
			...
			$context->addError(
				...
				PacketeryNette\Schema\Message::VALUE_OUT_OF_RANGE,
	...
	{
		if ($this->castTo) {
			if (PacketeryNette\Utils\Reflection::isBuiltinType($this->castTo)) {
				settype($value, $this->castTo);
			} else {
				$value = PacketeryNette\Utils\Arrays::toObject($value, new $this->castTo);
			}
		}
		...
		foreach ($this->asserts as $i => [$handler, $description]) {
			if (!$handler($value)) {
				...
				$context->addError(
					...
					PacketeryNette\Schema\Message::FAILED_ASSERTION,
