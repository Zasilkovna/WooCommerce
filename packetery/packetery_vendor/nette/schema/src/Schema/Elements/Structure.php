...
final class Structure implements Schema
{
	...
	{
		throw new PacketeryNette\InvalidStateException('Structure cannot have default value.');
	}
		...
		if ($extraKeys = array_keys(array_diff_key($value, $items))) {
			if ($this->otherItems) {
				...
				foreach ($extraKeys as $key) {
					$hint = PacketeryNette\Utils\Helpers::getSuggestion($keys, (string) $key);
					$context->addError(
						'Unexpected item %path%' . ($hint ? ", did you mean '%hint%'?" : '.'),
						PacketeryNette\Schema\Message::UNEXPECTED_ITEM,
						['hint' => $hint]
					)->path[] = $key;
				}
			}
