...
use PacketeryNette;
use PacketeryNette\Utils\DateTime;
...
final class Response implements IResponse
{
	 ...
	 * @return static
	 * @throws PacketeryNette\InvalidArgumentException  if code is invalid
	 * @throws PacketeryNette\InvalidStateException  if HTTP headers have been sent
	 */
	public function setCode(int $code, string $reason = null)
	{
		if ($code < 100 || $code > 599) {
			throw new PacketeryNette\InvalidArgumentException("Bad HTTP response '$code'.");
		}
		self::checkHeaders();
		$this->code = $code;
	...
	public function __destruct()
	{
		if (
			...
			echo PacketeryNette\Utils\Random::generate(2000, " \t\r\n"); // sends invisible garbage for IE
			self::$fixIE = false;
		}
	}
	...
	private function checkHeaders(): void
	{
		if (PHP_SAPI === 'cli') {
			...
			throw new PacketeryNette\InvalidStateException('Cannot send header after HTTP headers have been sent' . ($file ? " (output started at $file:$line)." : '.'));
			...
			trigger_error('Possible problem: you are sending a HTTP header while already having some data in output buffer. Try PacketeryTracy\OutputDebugger or start session earlier.');
		}
	}
}
