...
class ShippingRateFactoryTest extends TestCase {
	...
	public static function createShippingRatesProvider(): array {
		...
		$carrierOptionsCzPpMaxCartValue = new Carrier\Options(
			OptionPrefixer::getOptionId( $carrierCzPp->getId() ),
			[
				'id'                  => $carrierCzPp->getId(),
				'active'              => true,
		...
		return [
			'no customer country -> empty result'        => [
				...
				'cartWeight'                  => 1.0,
				'totalValue'                  => 100.0,
				'totalCartValueAfterDiscount' => 100.0,
				'globalMaxCartValue'          => null,
			...
			'carrier max cart value exceeded hides rate' => [
				'allowedCarrierNames'         => null,
				'methodId'                    => ShippingMethod::PACKETERY_METHOD_ID,
				'instanceId'                  => 1,
				'customerCountry'             => 'cz',
				'availableCarriers'           => [ [ $carrierCzPp, $carrierOptionsCzPpMaxCartValue ] ],
				'cartTotal'                   => 150.0,
				'cartWeight'                  => 1.0,
				'totalValue'                  => 150.0,
				'totalCartValueAfterDiscount' => 150.0,
				'globalMaxCartValue'          => null,
				'isAgeVerificationRequired'   => false,
				'disallowedRateIds'           => [],
				'oversized'                   => false,
	...
	public function testCreateShippingRates(
		...
		float $cartWeight,
		float $totalValue,
		float $totalCartValueAfterDiscount,
		?int $globalMaxCartValue,
		...
		$this->cartServiceMock->method( 'getTotalCartProductValueAfterDiscount' )->willReturn( $totalCartValueAfterDiscount );
		...
		$this->optionsProviderMock->method( 'getEffectiveMaxCartValueLimit' )->willReturnCallback(
			static function ( Carrier\Options $carrierOptions ) use ( $globalMaxCartValue ): ?int {
				return $carrierOptions->getMaxCartValue() ?? $globalMaxCartValue;
			}
		);
