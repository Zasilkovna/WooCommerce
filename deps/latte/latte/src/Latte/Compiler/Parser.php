...
class Parser
{
    ...
    public const CONTENT_HTML = Engine::CONTENT_HTML, CONTENT_XHTML = Engine::CONTENT_XHTML, CONTENT_XML = Engine::CONTENT_XML, CONTENT_TEXT = Engine::CONTENT_TEXT;
    /** @internal states */
    public const CONTEXT_NONE = 'none', CONTEXT_MACRO = 'macro', CONTEXT_HTML_TEXT = 'htmlText', CONTEXT_HTML_TAG = 'htmlTag', CONTEXT_HTML_ATTRIBUTE = 'htmlAttribute', CONTEXT_HTML_COMMENT = 'htmlComment', CONTEXT_HTML_CDATA = 'htmlCData';
    ...
    private function contextHtmlTag() : bool
    {
        ...
        if (!empty($matches['end'])) {
            ...
            $this->setContext(!$this->xmlMode && !$empty && \in_array($this->lastHtmlTag, ['script', 'style'], \true) ? self::CONTEXT_HTML_CDATA : self::CONTEXT_HTML_TEXT);
            return \true;
    ...
    {
        if (\in_array($type, [self::CONTENT_HTML, self::CONTENT_XHTML, self::CONTENT_XML], \true)) {
            $this->setContext(self::CONTEXT_HTML_TEXT);
