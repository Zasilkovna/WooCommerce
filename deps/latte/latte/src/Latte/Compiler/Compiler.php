...
class Compiler
{
    ...
    public const CONTENT_HTML = Engine::CONTENT_HTML, CONTENT_XHTML = Engine::CONTENT_XHTML, CONTENT_XML = Engine::CONTENT_XML, CONTENT_JS = Engine::CONTENT_JS, CONTENT_CSS = Engine::CONTENT_CSS, CONTENT_ICAL = Engine::CONTENT_ICAL, CONTENT_TEXT = Engine::CONTENT_TEXT;
    /** @internal Context-aware escaping HTML contexts */
    public const CONTEXT_HTML_TEXT = null, CONTEXT_HTML_TAG = 'Tag', CONTEXT_HTML_ATTRIBUTE = 'Attr', CONTEXT_HTML_ATTRIBUTE_JS = 'AttrJs', CONTEXT_HTML_ATTRIBUTE_CSS = 'AttrCss', CONTEXT_HTML_ATTRIBUTE_URL = 'AttrUrl', CONTEXT_HTML_ATTRIBUTE_UNQUOTED_URL = 'AttrUnquotedUrl', CONTEXT_HTML_COMMENT = 'Comment', CONTEXT_HTML_BOGUS_COMMENT = 'Bogus', CONTEXT_HTML_CSS = 'Css', CONTEXT_HTML_JS = 'Js', CONTEXT_XML_TEXT = self::CONTEXT_HTML_TEXT, CONTEXT_XML_TAG = self::CONTEXT_HTML_TAG, CONTEXT_XML_ATTRIBUTE = self::CONTEXT_HTML_ATTRIBUTE, CONTEXT_XML_COMMENT = self::CONTEXT_HTML_COMMENT, CONTEXT_XML_BOGUS_COMMENT = self::CONTEXT_HTML_BOGUS_COMMENT;
    /** @var string[] @internal */
    public $placeholders = [];
    /** @var string|null */
    ...
    private function buildClassBody(array $tokens) : string
    {
        ...
        foreach ($tokens as $this->position => $token) {
            if ($this->inHead && !($token->type === $token::COMMENT || $token->type === $token::MACRO_TAG && ($this->flags[$token->name] ?? null) & Macro::ALLOWED_IN_HEAD || $token->type === $token::TEXT && \trim($token->text) === '')) {
                $this->inHead = \false;
            }
        ...
        foreach ($this->constants as $name => $value) {
            $members[] = "\tprotected const {$name} = " . PhpHelpers::dump($value, \true) . ';';
        }
        foreach ($this->properties as $name => $value) {
            $members[] = "\tpublic \${$name} = " . PhpHelpers::dump($value, \true) . ';';
        }
     ...
     * @internal
     */
    public function addMethod(string $name, string $body, string $arguments = '', string $returns = '', ?string $comment = null) : void
    {
        $body = \trim($body);
    ...
    private function processMacroTag(Token $token) : void
    {
        ...
        if ($token->closing) {
            ...
            if (!$token->empty && ($this->flags[$token->name] ?? null) & Macro::AUTO_EMPTY) {
                $pos = $this->position;
                while (($t = $this->tokens[++$pos] ?? null) && ($t->type !== Token::MACRO_TAG || $t->name !== $token->name) && ($t->type !== Token::HTML_ATTRIBUTE_BEGIN || $t->name !== Parser::N_PREFIX . $token->name)) {
                }
                $token->empty = $t ? !$t->closing : \true;
                if ($token->empty) {
                    $tmp = \substr($token->text, 0, -1) . ' /}';
                    \trigger_error("Auto-empty behaviour is deprecated, replace {$token->text} with {$tmp} (on line {$this->getLine()})", \E_USER_DEPRECATED);
                }
            }
    ...
    {
        if (\in_array($this->context, [self::CONTEXT_HTML_COMMENT, self::CONTEXT_HTML_BOGUS_COMMENT], \true)) {
        ...
        if (!$htmlNode->closing) {
            ...
            if (\in_array($this->contentType, [self::CONTENT_HTML, self::CONTENT_XHTML], \true)) {
        ...
        } elseif ((($lower = \strtolower($htmlNode->name)) === 'script' || $lower === 'style') && (!isset($htmlNode->attrs['type']) || \preg_match('#(java|j|ecma|live)script|module|json|css|plain#i', $htmlNode->attrs['type']))) {
    ...
    private function processHtmlAttributeBegin(Token $token) : void
    {
        ...
        if (\in_array($this->contentType, [self::CONTENT_HTML, self::CONTENT_XHTML], \true) && (\in_array($lower, ['href', 'src', 'action', 'formaction'], \true) || $lower === 'data' && \strtolower($this->htmlNode->name) === 'object')) {
    ...
    private function processComment(Token $token) : void
    {
        ...
        $isLeftmost = \trim(\substr($this->output, $leftOfs)) === '';
    ...
    public function closeMacro(string $name, string $args = '', string $modifiers = '', bool $isRightmost = \false, ?string $nPrefix = null) : MacroNode
    {
        ...
        $this->output .= $node->content;
        $this->writeCode((string) $node->closingCode, $node->replaced, $isRightmost, \true);
        return $node;
    }
    ...
    public function writeAttrsMacro(string $html, ?bool $empty = null) : void
    {
        ...
        foreach ($this->macros as $name => $foo) {
            ...
            if ($empty) {
                \trigger_error("Unexpected n:{$attrName} on void element <{$this->htmlNode->name}> (on line {$this->getLine()}", \E_USER_WARNING);
            }
        ...
        if ($attrs) {
            throw new CompileException('Unknown attribute ' . Parser::N_PREFIX . \implode(' and ' . Parser::N_PREFIX, \array_keys($attrs)) . (($t = Helpers::getSuggestion(\array_keys($this->macros), \key($attrs))) ? ', did you mean ' . Parser::N_PREFIX . $t . '?' : ''));
        }
        if (!$this->htmlNode->closing) {
    ...
    public function expandMacro(string $name, string $args, string $modifiers = '', ?string $nPrefix = null) : MacroNode
    {
        if (empty($this->macros[$name])) {
            $hint = (($t = Helpers::getSuggestion(\array_keys($this->macros), $name)) ? ", did you mean {{$t}}?" : '') . (\in_array($this->context, [self::CONTEXT_HTML_JS, self::CONTEXT_HTML_CSS], \true) ? ' (in JavaScript or CSS, try to put a space after bracket or use n:syntax=off)' : '');
        ...
        if (\strpbrk($name, '=~%^&_')) {
            if (\in_array($this->context, [self::CONTEXT_HTML_ATTRIBUTE_URL, self::CONTEXT_HTML_ATTRIBUTE_UNQUOTED_URL], \true)) {
        ...
        throw new CompileException('Unknown ' . ($nPrefix ? 'attribute ' . Parser::N_PREFIX . ($nPrefix === MacroNode::PREFIX_NONE ? '' : "{$nPrefix}-") . $name : 'tag {' . $name . ($args ? " {$args}" : '') . '}'));
    }
    /**
    ...
    {
        return $node instanceof HtmlNode ? "</{$node->name}> for " . Parser::N_PREFIX . \implode(' and ' . Parser::N_PREFIX, \array_keys($node->macroAttrs)) : "{/{$node->name}}";
    }
    private function closeHtmlTag($token) : void
    {
        ...
        } elseif (\in_array($this->contentType, [self::CONTENT_HTML, self::CONTENT_XHTML], \true) && \in_array(\strtolower($this->htmlNode->name), ['script', 'style'], \true)) {
            \trigger_error("Unexpected {$token}, expecting </{$this->htmlNode->name}> (on line {$this->getLine()})", \E_USER_DEPRECATED);
        }
        $this->htmlNode = $this->htmlNode->parentNode;
    }
}
