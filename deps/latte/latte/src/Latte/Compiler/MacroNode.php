...
{
    use Strict;
    public const PREFIX_INNER = 'inner', PREFIX_TAG = 'tag', PREFIX_NONE = 'none';
    /** @var Macro */
    public $macro;
    /** @var string */
    public $name;
    ...
    {
        return $this->prefix ? Parser::N_PREFIX . ($this->prefix === self::PREFIX_NONE ? '' : $this->prefix . '-') . $this->name : '{' . $this->name . '}';
    }
    /**
     * @param  string[]  $names
     */
    ...
    {
        if ($parents && (!$this->parentNode || !\in_array($this->parentNode->name, $parents, \true))) {
