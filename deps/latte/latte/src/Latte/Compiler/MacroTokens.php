...
class MacroTokens extends TokenIterator
{
    ...
    public function parse(string $s) : array
    {
        self::$tokenizer = self::$tokenizer ?: new Tokenizer([
            ...
            self::T_KEYWORD => '(?:true|false|null|TRUE|FALSE|NULL|INF|NAN|and|or|xor|AND|OR|XOR|clone|new|instanceof|return|continue|break)(?!\\w)',
            // keyword
            self::T_CAST => '\\((?:expand|string|array|int|integer|float|bool|boolean|object)\\)',
            // type casting
            self::T_VARIABLE => '\\$\\w+',
    ...
    public function append($val, ?int $position = null)
    {
        if ($val != null) {
            // intentionally @
            \array_splice($this->tokens, $position ?? \count($this->tokens), 0, \is_array($val) ? [$val] : $this->parse($val));
        }
        return $this;
    }
    /**
