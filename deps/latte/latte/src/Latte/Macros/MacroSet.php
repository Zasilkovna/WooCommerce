...
class MacroSet implements \Packetery\Latte\Macro
{
    ...
    /** @var \Packetery\Latte\Compiler */
    private $compiler;
    /** @var array<string, array{string|callable|null, string|callable|null, string|callable|null}> */
    private $macros;
    public function __construct(\Packetery\Latte\Compiler $compiler)
    {
    ...
    public function addMacro(string $name, $begin, $end = null, $attr = null, ?int $flags = null) : self
    {
        ...
        foreach ([$begin, $end, $attr] as $arg) {
            if ($arg && !\is_string($arg)) {
                \Packetery\Latte\Helpers::checkCallback($arg);
            }
        }
    ...
    public function nodeOpened(MacroNode $node)
    {
        ...
        if ($attr && $node->prefix === $node::PREFIX_NONE) {
            $node->empty = \true;
            $node->context[1] = \Packetery\Latte\Compiler::CONTEXT_HTML_ATTRIBUTE;
            ...
            $node->context[1] = \Packetery\Latte\Compiler::CONTEXT_HTML_TEXT;
    ...
    private function compile(MacroNode $node, $def)
    {
        ...
        $writer = \Packetery\Latte\PhpWriter::using($node, $this->compiler);
        return \is_string($def) ? $writer->write($def) : $def($node, $writer);
    }
    public function getCompiler() : \Packetery\Latte\Compiler
    {
