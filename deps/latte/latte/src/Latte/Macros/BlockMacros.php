...
use Packetery\Latte\Runtime\Block;
use Packetery\Latte\Runtime\SnippetDriver;
...
class BlockMacros extends MacroSet
{
    ...
    public static function install(\Packetery\Latte\Compiler $compiler) : void
    {
        ...
        $me->addMacro('snippet', [$me, 'macroSnippet'], [$me, 'macroBlockEnd']);
        // must be before block
    ...
    public function finalize()
    {
        ...
        foreach ($this->placeholders as $key => [$index, $blockName]) {
            ...
            $compiler->placeholders[$key] = $block && !$block->hasParameters ? 'get_defined_vars()' : '[]';
        }
        $meta = [];
        foreach ($this->blocks as $layer => $blocks) {
            foreach ($blocks as $name => $block) {
                $compiler->addMethod($method = $this->generateMethodName($name), '?>' . $compiler->expandTokens($block->code) . '<?php', 'array $ʟ_args', 'void', $block->comment);
                $meta[$layer][$name] = $block->contentType === $compiler->getContentType() ? $method : [$method, $block->contentType];
            }
        }
        if ($meta) {
            $compiler->addConstant('BLOCKS', $meta);
        }
        return [($this->extends === null ? '' : '$this->parentName = ' . $this->extends . ';') . \implode('', $this->imports)];
    }
    /********************* macros ****************d*g**/
    /**
    ...
    {
        $node->validate(\true, [], \true);
        $node->replaced = \false;
        $tmp = $node->tokenizer->joinUntil('=');
        if ($node->tokenizer->isNext('=') && !$node->tokenizer->depth) {
            \trigger_error('The assignment in the {' . $node->name . ' ' . $tmp . '= ...} looks like an error.', \E_USER_NOTICE);
        }
        $node->tokenizer->reset();
        [$name, $mod] = $node->tokenizer->fetchWordWithModifier(['block', 'file', '#']);
        if (!$mod && \preg_match('~([\'"])[\\w-]+\\1$~DA', $name)) {
            \trigger_error("Change {include {$name}} to {include file {$name}} for clarity (on line {$node->startLine})", \E_USER_NOTICE);
        }
        if ($mod !== 'block' && $mod !== '#' && ($mod === 'file' || !$name || !\preg_match('~[\\w-]+$~DA', $name))) {
            return \false;
            // {include file}
        }
        ...
        if ($node->tokenizer->nextToken('from')) {
            ...
            return $writer->write('$this->createTemplate(%node.word, %node.array? + $this->params, "include")->renderToContentType(%raw, %word) %node.line;', $node->modifiers ? $writer->write('function ($s, $type) { $ʟ_fi = new LR\\FilterInfo($type); return %modifyContent($s); }') : PhpHelpers::dump($noEscape ? null : \implode('', $node->context)), $name);
        }
        $parent = $name === 'parent';
        if ($name === 'parent' || $name === 'this') {
            $item = $node->closest(['block', 'define'], function ($node) {
        ...
        $key = \uniqid() . '$iterator';
        // to fool CoreMacros::macroEndForeach
        $this->placeholders[$key] = [$this->index, $name];
        $phpName = $this->isDynamic($name) ? $writer->formatWord($name) : PhpHelpers::dump($name);
        return $writer->write('$this->renderBlock' . ($parent ? 'Parent' : '') . '(' . $phpName . ', ' . '%node.array? + ' . $key . ($node->modifiers ? ', function ($s, $type) { $ʟ_fi = new LR\\FilterInfo($type); return %modifyContent($s); }' : ($noEscape || $parent ? '' : ', ' . PhpHelpers::dump(\implode('', $node->context)))) . ') %node.line;');
    }
    /**
     * {includeblock "file"}
     * @deprecated
    ...
    {
        \trigger_error("Macro {includeblock} is deprecated, use {include {$node->args} with blocks} or similar macro {import} (on line {$node->startLine})", \E_USER_DEPRECATED);
        $node->replaced = \false;
        $node->validate(\true);
        return $writer->write('
			ob_start(function () {});
    ...
    public function macroBlock(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        if ($data->name === '') {
            ...
            $node->modifiers .= '|escape';
            $node->closingCode = $writer->write('<?php } finally { $ʟ_fi = new LR\\FilterInfo(%var); echo %modifyContent(ob_get_clean()); } ?>', \implode('', $node->context));
            return $writer->write('ob_start(function () {}) %node.line; try {');
        }
        if (Helpers::startsWith((string) $node->context[1], \Packetery\Latte\Compiler::CONTEXT_HTML_ATTRIBUTE)) {
            $node->context[1] = '';
            $node->modifiers .= '|escape';
        } elseif ($node->modifiers) {
            $node->modifiers .= '|escape';
        }
        $renderArgs = $writer->write('get_defined_vars()' . ($node->modifiers ? ', function ($s, $type) { $ʟ_fi = new LR\\FilterInfo($type); return %modifyContent($s); }' : ''));
        ...
        return $writer->write(($extendsCheck ? '' : 'if ($this->getParentName()) { return get_defined_vars(); } ') . '$this->renderBlock(%var, %raw) %node.line;', $data->name, $renderArgs);
    }
    /**
     * {define [local] name}
     */
    public function macroDefine(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        while ($tokens->isNext(...$tokens::SIGNIFICANT)) {
            ...
            $params[] = $writer->write('%raw = $ʟ_args[%var] ?? $ʟ_args[%var] ?? %raw;', $param, \count($params), \substr($param, 1), $default);
        ...
        return $extendsCheck ? '' : 'if ($this->getParentName()) { return get_defined_vars();} ';
    }
    private function beginDynamicBlockOrDefine(MacroNode $node, PhpWriter $writer, ?string $layer) : string
    {
        ...
        $data->after = function () use($node, $func) {
            ...
            $this->getCompiler()->addMethod($func, $this->getCompiler()->expandTokens("extract(\$ʟ_args); unset(\$ʟ_args);\n?>{$node->content}<?php"), 'array $ʟ_args', 'void', "{{$node->name} {$node->args}} on line {$node->startLine}");
            $node->content = '';
        };
        return $writer->write('$this->addBlock($ʟ_nm = %word, %var, [[$this, %var]], %var);', $data->name, \implode('', $node->context), $func, $layer);
    }
    /**
     * {snippet [name]}
     */
    public function macroSnippet(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        if ($node->prefix && $node->prefix !== $node::PREFIX_NONE) {
            \trigger_error("Use n:snippet instead of {$node->getNotation()}", \E_USER_DEPRECATED);
        }
        ...
        $data->after = function () use($node, $writer, $data, $block) {
            ...
            $node->content = $writer->write('<?php $this->global->snippetDriver->enter(%word, %var);
				try { ?>%raw<?php } finally { $this->global->snippetDriver->leave(); } ?>', $data->name, SnippetDriver::TYPE_STATIC, \preg_replace('#(?<=\\n)[ \\t]+$#D', '', $node->content));
            $this->extractMethod($node, $block);
            if ($node->prefix === MacroNode::PREFIX_NONE) {
        ...
        if ($node->prefix) {
            ...
            $node->attrCode = $writer->write("<?php echo ' {$this->snippetAttribute}=\"' . htmlspecialchars(\$this->global->snippetDriver->getHtmlId(%var)) . '\"' ?>", $data->name);
            return $writer->write('$this->renderBlock(%var, [], null, %var)', $data->name, Template::LAYER_SNIPPET);
        }
        return $writer->write("?>\n<div {$this->snippetAttribute}=\"<?php echo htmlspecialchars(\$this->global->snippetDriver->getHtmlId(%0_var)) ?>\">" . '<?php $this->renderBlock(%0_var, [], null, %1_var) %node.line; ?>' . "\n</div><?php ", $data->name, Template::LAYER_SNIPPET);
    }
    private function beginDynamicSnippet(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        if ($node->prefix) {
            ...
            $node->attrCode = $writer->write("<?php echo ' {$this->snippetAttribute}=\"' . htmlspecialchars(\$this->global->snippetDriver->getHtmlId(\$ʟ_nm = %word)) . '\"' ?>", $data->name);
            return $writer->write('$this->global->snippetDriver->enter($ʟ_nm, %var) %node.line; try {', SnippetDriver::TYPE_DYNAMIC);
        }
        $node->closingCode .= "\n</div>";
        return $writer->write("?>\n<div {$this->snippetAttribute}=\"" . '<?php echo htmlspecialchars($this->global->snippetDriver->getHtmlId($ʟ_nm = %word)) ?>"' . '><?php $this->global->snippetDriver->enter($ʟ_nm, %var) %node.line; try {', $data->name, SnippetDriver::TYPE_DYNAMIC);
    }
    /**
     * {snippetArea [name]}
     */
    public function macroSnippetArea(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        $block = $this->addBlock($node, Template::LAYER_SNIPPET);
        $data->after = function () use($node, $writer, $data, $block) {
            $node->content = $writer->write('<?php $this->global->snippetDriver->enter(%var, %var);
				try { ?>%raw<?php } finally { $this->global->snippetDriver->leave(); } ?>', $data->name, SnippetDriver::TYPE_AREA, \preg_replace('#(?<=\\n)[ \\t]+$#D', '', $node->content));
            $this->extractMethod($node, $block);
        };
    ...
    {
        $data = $node->data;
        if ($layer === Template::LAYER_SNIPPET ? isset($this->blocks[$layer][$data->name]) : isset($this->blocks[Template::LAYER_LOCAL][$data->name]) || isset($this->blocks[$this->index][$data->name])) {
    ...
    private function extractMethod(MacroNode $node, Block $block, ?string $params = null) : void
    {
        if (\preg_match('#\\$|n:#', $node->content)) {
            $node->content = '<?php extract(' . ($node->name === 'block' && $node->closest(['embed']) ? 'end($this->varStack)' : '$this->params') . ');' . ($params ?? 'extract($ʟ_args);') . 'unset($ʟ_args);?>' . $node->content;
        }
    ...
    public function macroEmbed(MacroNode $node, PhpWriter $writer) : void
    {
        ...
        [$name, $mod] = $node->tokenizer->fetchWordWithModifier(['block', 'file']);
        if (!$mod && \preg_match('~([\'"])[\\w-]+\\1$~DA', $name)) {
            \trigger_error("Change {embed {$name}} to {embed file {$name}} for clarity (on line {$node->startLine})", \E_USER_NOTICE);
        }
    ...
    public function macroIfset(MacroNode $node, PhpWriter $writer)
    {
        ...
        while ([$name, $block] = $node->tokenizer->fetchWordWithModifier(['block', '#'])) {
            $list[] = $block || \preg_match('~\\w[\\w-]*$~DA', $name) ? '$this->hasBlock(' . $writer->formatWord($name) . ')' : 'isset(' . $writer->formatArgs(new \Packetery\Latte\MacroTokens($name)) . ')';
        }
    ...
    {
        $name = 'block' . \ucfirst(\trim(\preg_replace('#\\W+#', '_', $blockName), '_'));
