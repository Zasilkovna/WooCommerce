...
class BlockMacros extends MacroSet
{
    ...
    public function macroInclude(MacroNode $node, PhpWriter $writer)
    {
        ...
        return $writer->write('$this->renderBlock' . ($parent ? 'Parent' : '') . '($ʟ_nm = ' . $phpName . ', ' . '%node.array? + ' . $key . ($node->modifiers ? ', function ($s, $type) { $ʟ_fi = new LR\\FilterInfo($type); return %modifyContent($s); }' : ($noEscape || $parent ? '' : ', ' . PhpHelpers::dump(\implode($node->context)))) . ') %node.line;');
    }
    /**
     * {includeblock "file"}
     * @deprecated
    ...
    public function macroIncludeBlock(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        return $writer->write('ob_start(function () {});
			...
			echo rtrim(ob_get_clean());', \implode($node->context));
    }
    /**
     * {import "file"}
     */
    ...
    public function macroBlock(MacroNode $node, PhpWriter $writer) : string
    {
        ...
        if ($data->name === '') {
            ...
            $node->modifiers .= '|escape';
            $node->closingCode = $writer->write('<?php $ʟ_fi = new LR\\FilterInfo(%var); echo %modifyContent(ob_get_clean()); ?>', \implode($node->context));
    ...
    public function macroIfset(MacroNode $node, PhpWriter $writer)
    {
        ...
        while ([$name, $block] = $node->tokenizer->fetchWordWithModifier('block')) {
            $list[] = $block || \preg_match('~#|[\\w-]+$~DA', $name) ? '$this->hasBlock(' . $writer->formatWord(\ltrim($name, '#')) . ')' : 'isset(' . $writer->formatArgs(new \Packetery\Latte\MacroTokens($name)) . ')';
        }
