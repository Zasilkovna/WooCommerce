...
class Engine
{
    ...
    public function renderToString(string $name, $params = [], ?string $block = null) : string
    {
        ...
        ($this->probe)($template);
        return $template->capture(function () use($template, $block) {
            $template->render($block);
        });
    }
    /**
    ...
    private function loadTemplate(string $name) : void
    {
        if (!$this->tempDirectory) {
            ...
            if (@eval(\substr($code, 5)) === \false) {
                // @ is escalated to exception, substr removes <?php
    ...
    {
        return $this->autoRefresh && $this->getLoader()->isExpired($name, (int) @\filemtime($file));
        // @ - file may not exist
    }
    public function getCacheFile(string $name) : string
    {
        $hash = \substr($this->getTemplateClass($name), 8);
        $base = \preg_match('#([/\\\\][\\w@.-]{3,35}){1,3}$#D', $name, $m) ? \preg_replace('#[^\\w@.-]+#', '-', \substr($m[0], 1)) . '--' : '';
    ...
    /** @return static */
    public function setSandboxMode(bool $on = \true)
    {
        $this->sandboxed = $on;
        return $this;
    }
    ...
    private function processParams($params) : array
    {
        ...
        foreach ($methods as $method) {
            if (\PHP_VERSION_ID >= 80000 && $method->getAttributes(Attributes\TemplateFilter::class) || \strpos((string) $method->getDocComment(), '@filter')) {
                $this->addFilter($method->name, [$params, $method->name]);
            }
            if (\PHP_VERSION_ID >= 80000 && $method->getAttributes(Attributes\TemplateFunction::class) || \strpos((string) $method->getDocComment(), '@function')) {
                $this->addFunction($method->name, [$params, $method->name]);
            }
        }
        return \array_filter((array) $params, function ($key) {
            return $key[0] !== "\x00";
