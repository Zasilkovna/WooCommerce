...
class Template
{
    ...
    public const LAYER_TOP = 0, LAYER_SNIPPET = 'snippet', LAYER_LOCAL = 'local';
    ...
    {
        \trigger_error(__METHOD__ . '() is deprecated, use getParameters()', \E_USER_DEPRECATED);
        if (!\array_key_exists($name, $this->params)) {
            \trigger_error("The variable '{$name}' does not exist in template.", \E_USER_NOTICE);
        }
    ...
    private function doRender(?string $block = null) : bool
    {
        ...
        if ($this->referenceType === 'import') {
            if ($this->parentName) {
                throw new \Packetery\Latte\RuntimeException('Imported template cannot use {extends} or {layout}, use {import}');
            }
        ...
        } elseif (isset($this->global->snippetDriver) && $this->global->snippetDriver->renderSnippets($this->blocks[self::LAYER_SNIPPET], $this->params)) {
            // nothing
        } else {
            return \false;
        }
    ...
    public function createTemplate(string $name, array $params, string $referenceType) : self
    {
        ...
        $referred = $referenceType === 'sandbox' ? (clone $this->engine)->setSandboxMode()->createTemplate($name, $params) : $this->engine->createTemplate($name, $params);
        $referred->referringTemplate = $this;
        $referred->referenceType = $referenceType;
        $referred->global = $this->global;
        if (\in_array($referenceType, ['extends', 'includeblock', 'import', 'embed'], \true)) {
    ...
    {
        $this->filter(function () use($block) {
            $this->render($block);
        }, $mod, static::CONTENT_TYPE, "'{$this->name}'");
    }
    /** @internal */
    ...
    {
        $block = $layer ? $this->blocks[$layer][$name] ?? null : $this->blocks[self::LAYER_LOCAL][$name] ?? $this->blocks[self::LAYER_TOP][$name] ?? null;
        if (!$block) {
            $hint = ($t = \Packetery\Latte\Helpers::getSuggestion($this->getBlockNames($layer), $name)) ? ", did you mean '{$t}'?" : '.';
            $name = $layer ? "{$layer} {$name}" : $name;
            throw new \Packetery\Latte\RuntimeException("Cannot include undefined block '{$name}'{$hint}");
        }
        $this->filter(function () use($block, $params) : void {
            \reset($block->functions)($params);
        }, $mod, $block->contentType, "block {$name}");
    }
    /**
    ...
    public function renderBlockParent(string $name, array $params) : void
    {
        ...
        if (!$block || ($function = \next($block->functions)) === \false) {
            throw new \Packetery\Latte\RuntimeException("Cannot include undefined parent block '{$name}'.");
        }
        $function($params);
    ...
    protected function addBlock(string $name, string $contentType, array $functions, $layer = null) : void
    {
        ...
        if ($block->contentType === null) {
            ...
            throw new \Packetery\Latte\RuntimeException(\sprintf("Overridden block {$name} with content type %s by incompatible type %s.", \strtoupper($contentType), \strtoupper($block->contentType)));
        }
    ...
    private function filter(callable $function, $mod, string $contentType, string $name) : void
    {
        if ($mod === null || $mod === $contentType) {
            ...
            throw new \Packetery\Latte\RuntimeException(\sprintf("Including {$name} with content type %s into incompatible type %s.", \strtoupper($contentType), \strtoupper($mod)));
        }
    }
    /**
    ...
    protected function call($callable)
    {
        if (!\is_callable($callable)) {
            throw new \Packetery\Latte\SecurityViolationException('Invalid callable.');
        } elseif (\is_string($callable)) {
            $parts = \explode('::', $callable);
            $allowed = \count($parts) === 1 ? $this->policy->isFunctionAllowed($parts[0]) : $this->policy->isMethodAllowed(...$parts);
        } elseif (\is_array($callable)) {
            $allowed = $this->policy->isMethodAllowed(\is_object($callable[0]) ? \get_class($callable[0]) : $callable[0], $callable[1]);
        } elseif (\is_object($callable)) {
            $allowed = $callable instanceof \Closure ? \true : $this->policy->isMethodAllowed(\get_class($callable), '__invoke');
        } else {
            $allowed = \false;
        }
        if (!$allowed) {
            \is_callable($callable, \false, $text);
            throw new \Packetery\Latte\SecurityViolationException("Calling {$text}() is not allowed.");
        }
        return $callable;
    }
    /**
    ...
    protected function prop($obj, $prop)
    {
        ...
        if (\is_string($class) && !$this->policy->isPropertyAllowed($class, (string) $prop)) {
            throw new \Packetery\Latte\SecurityViolationException("Access to '{$prop}' property on a {$class} object is not allowed.");
        }
        return $obj;
    }
    /**
    ...
    {
        if ($name === 'blocks') {
            // compatibility with nette/application < 3.0.8
