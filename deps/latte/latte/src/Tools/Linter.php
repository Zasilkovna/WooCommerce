...
namespace Packetery\Latte\Tools;
...
final class Linter
{
    ...
    /** @var bool */
    private $debug;
    /** @var \Packetery\Latte\Engine|null */
    private $engine;
    ...
    public function scanDirectory(string $dir) : bool
    {
        ...
        $this->engine->setLoader(new \Packetery\Latte\Loaders\StringLoader());
        $counter = 0;
        $success = \true;
        foreach ($it as $file) {
    ...
    private function createEngine() : \Packetery\Latte\Engine
    {
        $engine = new \Packetery\Latte\Engine();
    ...
    public function lintLatte(string $file) : bool
    {
        \set_error_handler(function (int $severity, string $message) use($file) {
            if (\in_array($severity, [\E_USER_DEPRECATED, \E_USER_WARNING, \E_USER_NOTICE], \true)) {
        ...
        } catch (\Packetery\Latte\CompileException $e) {
            if ($this->debug) {
                echo $e;
            }
    ...
    private function lintPHP(string $code) : ?string
    {
        ...
        \fwrite($stdin, $code);
        \fseek($stdin, 0);
        $process = \proc_open($php . ' -l -d display_errors=1', [$stdin, ['pipe', 'w'], ['pipe', 'w']], $pipes, null, null, ['bypass_shell' => \true]);
