...
class TracyExtension extends Nette\DI\CompilerExtension
{
    ...
    public function afterCompile(Nette\PhpGenerator\ClassType $class)
    {
        ...
        foreach ($options as $key => $value) {
            if ($value !== null) {
                $tbl = ['keysToHide' => 'array_push(\\Packetery\\Tracy\\Debugger::getBlueScreen()->keysToHide, ... ?)', 'fromEmail' => 'if ($logger instanceof \\Packetery\\Tracy\\Logger) $logger->fromEmail = ?', 'emailSnooze' => 'if ($logger instanceof \\Packetery\\Tracy\\Logger) $logger->emailSnooze = ?'];
                $initialize->addBody($builder->formatPhp(($tbl[$key] ?? '\\Packetery\\Tracy\\Debugger::$' . $key . ' = ?') . ';', Nette\DI\Helpers::filterArguments([$value])));
            }
        }
        if ($this->config->netteMailer && $builder->getByType(Nette\Mail\IMailer::class)) {
            $initialize->addBody($builder->formatPhp('if ($logger instanceof \\Packetery\\Tracy\\Logger) $logger->mailer = ?;', [[new Statement(\Packetery\Tracy\Bridges\Nette\MailSender::class, ['fromEmail' => $this->config->fromEmail]), 'send']]));
        }
        if ($this->debugMode) {
            ...
            if (!$this->cliMode && \Packetery\Tracy\Debugger::getSessionStorage() instanceof \Packetery\Tracy\NativeSession && ($name = $builder->getByType(Nette\Http\Session::class))) {
                $initialize->addBody('$this->getService(?)->start();', [$name]);
                $initialize->addBody('\\Packetery\\Tracy\\Debugger::dispatch();');
            }
        }
        ...
        if (($dir = \Packetery\Tracy\Debugger::$logDirectory) && !\is_writable($dir)) {
