...
class Finder implements \IteratorAggregate, \Countable
{
    ...
    {
        if ($this->paths) {
            throw new \Packetery\Nette\InvalidStateException('Directory to search has already been specified.');
        }
    ...
    {
        \trigger_error('\\Packetery\\Nette\\Utils\\Finder::count is deprecated.', \E_USER_DEPRECATED);
    ...
    {
        if (!$this->paths) {
            throw new \Packetery\Nette\InvalidStateException('Call in() or from() to specify directory to search.');
    ...
    public function size(string $operator, ?int $size = null) : self
    {
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?((?:\\d*\\.)?\\d+)\\s*(K|M|G|)B?$#Di', $operator, $matches)) {
                throw new \Packetery\Nette\InvalidArgumentException('Invalid size predicate format.');
            }
    ...
    public function date(string $operator, $date = null) : self
    {
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?(.+)$#Di', $operator, $matches)) {
                throw new \Packetery\Nette\InvalidArgumentException('Invalid date predicate format.');
            }
    ...
    public static function compare($l, string $operator, $r) : bool
    {
        switch ($operator) {
            ...
            case '<>':
                return $l != $r;
            default:
                throw new \Packetery\Nette\InvalidArgumentException("Unknown operator {$operator}.");
        }
    }
    ...
    {
        return isset(self::$extMethods[$name]) ? self::$extMethods[$name]($this, ...$args) : \Packetery\Nette\Utils\ObjectHelpers::strictCall(static::class, $name, \array_keys(self::$extMethods));
    }
    /** @deprecated */
