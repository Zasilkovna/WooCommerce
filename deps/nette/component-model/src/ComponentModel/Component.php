...
abstract class Component implements IComponent
{
     ...
     */
    public final function lookup(?string $type, bool $throw = \true) : ?IComponent
    {
        if (!isset($this->monitors[$type])) {
            ...
            if ($obj) {
                ...
                $this->monitors[$type] = [null, null, null, []];
                // not found
            }
        }
        if ($throw && $this->monitors[$type][0] === null) {
            $message = $this->name !== null ? "Component '{$this->name}' is not attached to '{$type}'." : "Component of type '" . static::class . "' is not attached to '{$type}'.";
            throw new \Packetery\Nette\InvalidStateException($message);
        }
     ...
     */
    public final function lookupPath(?string $type = null, bool $throw = \true) : ?string
    {
     ...
     */
    public final function monitor(string $type, ?callable $attached = null, ?callable $detached = null) : void
    {
        if (\func_num_args() === 1) {
            $attached = [$this, 'attached'];
            $detached = [$this, 'detached'];
        }
        if (($obj = $this->lookup($type, \false)) && $attached && !\in_array([$attached, $detached], $this->monitors[$type][3], \true)) {
            $attached($obj);
        }
        $this->monitors[$type][3][] = [$attached, $detached];
        // mark as monitored
    }
    /**
     * Stops monitoring of ancestors.
     */
    public final function unmonitor(string $type) : void
    {
     ...
     */
    public final function getParent() : ?IContainer
    {
        return $this->parent;
    }
    /**
    ...
    public function setParent(?IContainer $parent, ?string $name = null)
    {
        ...
        if ($this->parent !== null && $parent !== null) {
            throw new \Packetery\Nette\InvalidStateException("Component '{$this->name}' already has a parent.");
        }
        // remove from parent?
        if ($parent === null) {
     ...
     * override this method to disallow a parent change by throwing an \Packetery\Nette\InvalidStateException
     * @throws \Packetery\Nette\InvalidStateException
     */
    ...
    private function refreshMonitors(int $depth, ?array &$missing = null, array &$listeners = []) : void
    {
        ...
        if ($depth === 0) {
            ...
            foreach ($listeners as $item) {
                if ($item[0] && !\in_array($item, $prev, \true)) {
                    $item[0]($item[1]);
                    $prev[] = $item;
                }
            }
    ...
    {
        throw new \Packetery\Nette\NotImplementedException('Object serialization is not supported by class ' . static::class);
    }
    /**
    ...
    {
        throw new \Packetery\Nette\NotImplementedException('Object unserialization is not supported by class ' . static::class);
    }
}
