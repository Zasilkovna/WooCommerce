...
final class Random
{
    ...
    public static function generate(int $length = 10, string $charlist = '0-9a-z') : string
    {
        ...
        if ($length < 1) {
            throw new \Packetery\Nette\InvalidArgumentException('Length must be greater than zero.');
        } elseif ($chLen < 2) {
            throw new \Packetery\Nette\InvalidArgumentException('Character list must contain at least two chars.');
        }
        $res = '';
        for ($i = 0; $i < $length; $i++) {
