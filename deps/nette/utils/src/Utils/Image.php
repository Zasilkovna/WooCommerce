...
class Image
{
    ...
    public static function fromFile(string $file, ?int &$type = null)
    {
        if (!\extension_loaded('gd')) {
            throw new \Packetery\Nette\NotSupportedException('PHP extension GD is not loaded.');
        }
    ...
    public static function fromBlank(int $width, int $height, ?array $color = null)
    {
        ...
        if ($width < 1 || $height < 1) {
            throw new \Packetery\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
        }
    ...
    public static function typeToExtension(int $type) : string
    {
        if (!isset(self::Formats[$type])) {
            throw new \Packetery\Nette\InvalidArgumentException("Unsupported image type '{$type}'.");
        }
    ...
    public static function extensionToType(string $extension) : int
    {
        ...
        if (!isset($extensions[$extension])) {
            throw new \Packetery\Nette\InvalidArgumentException("Unsupported file extension '{$extension}'.");
        }
    ...
    protected function setImageResource($image)
    {
        if (!$image instanceof \GdImage && !(\is_resource($image) && \get_resource_type($image) === 'gd')) {
            throw new \Packetery\Nette\InvalidArgumentException('Image is not valid.');
        }
        $this->image = $image;
        return $this;
    }
    ...
    public static function calculateSize(int $srcWidth, int $srcHeight, $newWidth, $newHeight, int $flags = self::FIT) : array
    {
        ...
        if ($flags & self::STRETCH) {
            ...
            if (!$newWidth || !$newHeight) {
                throw new \Packetery\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
            }
            ...
            // proportional
            if (!$newWidth && !$newHeight) {
                throw new \Packetery\Nette\InvalidArgumentException('At least width or height must be specified.');
            }
            $scale = [];
            if ($newWidth > 0) {
                // fit width
     ...
     * @return mixed
     * @throws \Packetery\Nette\MemberAccessException
     */
    ...
    private static function isPercent(&$num) : bool
    {
        ...
        throw new \Packetery\Nette\InvalidArgumentException("Expected dimension in int|string, '{$num}' given.");
    }
    /**
    ...
    {
        throw new \Packetery\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
    }
}
