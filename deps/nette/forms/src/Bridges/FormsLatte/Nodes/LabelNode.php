...
class LabelNode extends StatementNode
{
    ...
    {
        return $context->format('echo ($ʟ_label = \\Packetery\\Nette\\Bridges\\FormsLatte\\Runtime::item(%node, $this->global)->' . ($this->part ? 'getLabelPart(%node)' : 'getLabel()') . ')' . ($this->attributes->items ? '?->addAttributes(%2.node)' : '') . ($this->void ? ' %3.line;' : '?->startTag() %3.line; %4.node echo $ʟ_label?->endTag() %5.line;'), $this->name, $this->part, $this->attributes, $this->position, $this->content, $this->endLine);
    }
