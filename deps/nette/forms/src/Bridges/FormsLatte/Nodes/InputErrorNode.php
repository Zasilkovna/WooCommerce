...
use Packetery\Latte\Compiler\Nodes\Php\Expression\VariableNode;
...
class InputErrorNode extends StatementNode
{
    ...
    public static function create(Tag $tag) : static
    {
        ...
        $node = new static();
        if ($tag->parser->isEnd()) {
            \trigger_error("Missing argument in {inputError} (on line {$tag->position->line})", \E_USER_DEPRECATED);
    ...
    {
        return $context->format('echo %escape(\\Packetery\\Nette\\Bridges\\FormsLatte\\Runtime::item(%node, $this->global)->getError()) %line;', $this->name, $this->position);
    }
