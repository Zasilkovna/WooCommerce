...
class RadioList extends ChoiceControl
{
    ...
    public function getControl() : Html
    {
        ...
        return $this->container->setHtml(\Packetery\Nette\Forms\Helpers::createInputList($this->translate($items), \array_merge($input->attrs, ['id:' => $ids, 'checked?' => $this->value, 'disabled:' => $this->disabled, 'data-nette-rules:' => [\key($items) => $input->attrs['data-nette-rules']]]), ['for:' => $ids] + $this->itemLabel->attrs, $this->separator));
    }
    ...
    public function getControlPart($key = null) : Html
    {
        ...
        return parent::getControl()->addAttributes(['id' => $this->getHtmlId() . '-' . $key, 'checked' => \in_array($key, (array) $this->value, \true), 'disabled' => \is_array($this->disabled) ? isset($this->disabled[$key]) : $this->disabled, 'value' => $key]);
    }
