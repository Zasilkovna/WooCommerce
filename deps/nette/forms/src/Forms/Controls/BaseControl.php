...
use Packetery\Nette\Forms\Rules;
...
abstract class BaseControl extends \Packetery\Nette\ComponentModel\Component implements Control
{
    ...
    /** @var Rules */
    private $rules;
    /** @var \Packetery\Nette\Localization\Translator|bool|null */
    private $translator = \true;
    // means autodetect
    ...
    {
        return $this->control->name ?? \Packetery\Nette\Forms\Helpers::generateHtmlName($this->lookupPath(Form::class));
    }
    ...
    {
        return $this->disabled === \true;
    }
    /**
     * Sets whether control value is excluded from $form->getValues() result.
     * @return static
     */
    public function setOmitted(bool $value = \true)
    {
        $this->omitted = $value;
        return $this;
    }
    ...
    public function getControl()
    {
        ...
        return $el->addAttributes(['name' => $this->getHtmlName(), 'id' => $this->getHtmlId(), 'required' => $this->isRequired(), 'disabled' => $this->isDisabled(), 'data-nette-rules' => \Packetery\Nette\Forms\Helpers::exportRules($this->rules) ?: null]);
    }
    /**
    ...
    public function getLabel($caption = null)
    {
        ...
        $label->setText($translator && !$caption instanceof \Packetery\Nette\HtmlStringable ? $translator->translate($caption) : $caption);
        return $label;
    }
    ...
    public function getHtmlId()
    {
        if (!isset($this->control->id)) {
            $form = $this->getForm();
            $prefix = $form instanceof \Packetery\Nette\Application\UI\Form || $form->getName() === null ? '' : $form->getName() . '-';
     ...
     * @return static
     */
    public function setAttribute(string $name, $value = \true)
    {
    ...
    public function translate($value, ...$parameters)
    {
        if ($translator = $this->getTranslator()) {
            ...
            foreach ($tmp[0] as &$v) {
                if ($v != null && !$v instanceof \Packetery\Nette\HtmlStringable) {
                    // intentionally ==
