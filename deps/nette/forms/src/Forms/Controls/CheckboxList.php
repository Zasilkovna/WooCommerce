...
class CheckboxList extends MultiChoiceControl
{
    ...
    {
        $data = $this->getForm()->getHttpData(\Packetery\Nette\Forms\Form::DATA_TEXT, \substr($this->getHtmlName(), 0, -2));
        $data = $data === null ? $this->getHttpData(\Packetery\Nette\Forms\Form::DATA_TEXT) : \explode(',', $data);
    ...
    public function getControl() : Html
    {
        ...
        $items = $this->getItems();
        \reset($items);
        return $this->container->setHtml(\Packetery\Nette\Forms\Helpers::createInputList($this->translate($items), \array_merge($input->attrs, ['id' => null, 'checked?' => $this->value, 'disabled:' => $this->disabled, 'required' => null, 'data-nette-rules:' => [\key($items) => $input->attrs['data-nette-rules']]]), $this->itemLabel->attrs, $this->separator));
    }
    ...
    public function getControlPart($key = null) : Html
    {
        ...
        return parent::getControl()->addAttributes(['id' => $this->getHtmlId() . '-' . $key, 'checked' => \in_array($key, (array) $this->value, \true), 'disabled' => \is_array($this->disabled) ? isset($this->disabled[$key]) : $this->disabled, 'required' => null, 'value' => $key]);
    }
    public function getLabelPart($key = null) : Html
    {
        $itemLabel = clone $this->itemLabel;
        return \func_num_args() ? $itemLabel->setText($this->translate($this->items[$key]))->for($this->getHtmlId() . '-' . $key) : $this->getLabel();
    }
    /**
