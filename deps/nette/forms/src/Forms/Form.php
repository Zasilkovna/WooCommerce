...
class Form extends Container implements \Packetery\Nette\HtmlStringable
{
    ...
    public $onRender = [];
    /** @internal @var \Packetery\Nette\Http\IRequest  used only by standalone form */
    public $httpRequest;
    /** @var bool */
    ...
    /** @var FormRenderer */
    private $renderer;
    /** @var \Packetery\Nette\Localization\Translator */
    private $translator;
    /** @var ControlGroup[] */
    private $groups = [];
    /** @var array */
    ...
    public function __construct(string $name = null)
    {
        ...
        $this->monitor(self::class, function () : void {
            throw new \Packetery\Nette\InvalidStateException('Nested forms are forbidden.');
        });
    }
    /**
     * Returns self.
     * @return static
     */
    public function getForm(bool $throw = \true) : self
    {
        return $this;
    }
    /**
    ...
    public function setMethod(string $method)
    {
        if ($this->httpData !== null) {
            throw new \Packetery\Nette\InvalidStateException(__METHOD__ . '() must be called until the form is empty.');
        }
     ...
     * @return static
     */
    public function setHtmlAttribute(string $name, $value = \true)
    {
    ...
    public function addGroup($caption = null, bool $setAsCurrent = \true) : ControlGroup
    {
        ...
        $group->setOption('visual', \true);
        if ($setAsCurrent) {
            $this->setCurrentGroup($group);
        }
        return !\is_scalar($caption) || isset($this->groups[$caption]) ? $this->groups[] = $group : ($this->groups[$caption] = $group);
    }
    /**
    ...
    public function removeGroup($name) : void
    {
        ...
        } elseif ($name instanceof ControlGroup && \in_array($name, $this->groups, \true)) {
            $group = $name;
            $name = \array_search($group, $this->groups, \true);
        } else {
            throw new \Packetery\Nette\InvalidArgumentException("Group not found in form '{$this->name}'");
        }
    ...
    public function getHttpData(int $type = null, string $htmlName = null)
    {
        if ($this->httpData === null) {
            if (!$this->isAnchored()) {
                throw new \Packetery\Nette\InvalidStateException('Form is not anchored and therefore can not determine whether it was submitted.');
            }
    ...
    protected function receiveHttpData() : ?array
    {
        ...
        if ($httpRequest->isMethod('post')) {
            ...
            $data = \Packetery\Nette\Utils\Arrays::mergeTree($httpRequest->getPost(), $httpRequest->getFiles());
        } else {
    ...
    {
        $toggles = [];
        foreach ($this->getComponents(\true, Controls\BaseControl::class) as $control) {
    ...
    public static function initialize(bool $reinit = \false) : void
    {
        ...
        self::$defaultHttpRequest = (new \Packetery\Nette\Http\RequestFactory())->fromGlobals();
        if (\PHP_SAPI !== 'cli') {
