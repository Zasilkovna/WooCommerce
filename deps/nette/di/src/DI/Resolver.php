...
use Packetery\Nette\Utils\Callback;
...
class Resolver
{
    ...
    public function resolveEntityType(Statement $statement) : ?string
    {
        ...
        if (\is_array($entity)) {
            ...
            $type = \Packetery\Nette\Utils\Type::fromReflection($reflection) ?? ($annotation = Helpers::getReturnTypeAnnotation($reflection));
    ...
    private static function autowireArgument(\ReflectionParameter $parameter, callable $getter)
    {
        ...
        $type = \Packetery\Nette\Utils\Type::fromReflection($parameter);
    ...
    private static function isArrayOf(\ReflectionParameter $parameter, ?Nette\Utils\Type $type) : ?string
    {
        ...
        return $method instanceof \ReflectionMethod && $type && $type->getSingleName() === 'array' && \preg_match('#@param[ \\t]+(?|([\\w\\\\]+)\\[\\]|array<int,\\s*([\\w\\\\]+)>)[ \\t]+\\$' . $parameter->name . '#', (string) $method->getDocComment(), $m) && ($itemType = Reflection::expandClassName($m[1], $method->getDeclaringClass())) && (\class_exists($itemType) || \interface_exists($itemType)) ? $itemType : null;
    }
}
