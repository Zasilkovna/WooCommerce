...
class Resolver
{
    ...
    public function resolveEntityType(Statement $statement) : ?string
    {
        ...
        if (\is_array($entity)) {
            ...
            try {
                ...
                $reflection = \Packetery\Nette\Utils\Callback::toReflection($entity[0] === '' ? $entity[1] : $entity);
            ...
            if (isset($e) || $refClass && (!$reflection->isPublic() || $refClass->isTrait() && !$reflection->isStatic())) {
                throw new ServiceCreationException(\sprintf('Method %s() is not callable.', \Packetery\Nette\Utils\Callback::toString($entity)), 0, $e ?? null);
            }
            $this->addDependency($reflection);
            $type = Helpers::getReturnType($reflection);
            if ($type && !\class_exists($type) && !\interface_exists($type)) {
                throw new ServiceCreationException(\sprintf("Class or interface '%s' not found. Is return type of %s() correct?", $type, \Packetery\Nette\Utils\Callback::toString($entity)));
            }
            return $type;
            ...
            if (!\class_exists($entity)) {
                throw new ServiceCreationException(\interface_exists($entity) ? "Interface {$entity} can not be used as 'factory', did you mean 'implement'?" : "Class {$entity} not found.");
            }
            return $entity;
        }
        return null;
                ...
                switch (\true) {
                    ...
                    case $entity[0] instanceof Reference:
                        if ($entity[1][0] === '$') {
                            ...
                            Validators::assert($arguments, 'list:0..1', "setup arguments for '" . \Packetery\Nette\Utils\Callback::toString($entity) . "'");
    ...
    private static function autowireArgument(\ReflectionParameter $parameter, callable $getter)
    {
        ...
        } elseif ($method instanceof \ReflectionMethod && $type === 'array' && \preg_match('#@param[ \\t]+([\\w\\\\]+)\\[\\][ \\t]+\\$' . $parameter->name . '#', (string) $method->getDocComment(), $m) && ($itemType = Reflection::expandClassName($m[1], $method->getDeclaringClass())) && (\class_exists($itemType) || \interface_exists($itemType))) {
