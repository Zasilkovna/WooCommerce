...
class Container
{
    ...
    {
        throw new \Packetery\Nette\InvalidStateException(\sprintf("Parameter '%s' not found. Check if 'di › export › parameters' is enabled.", $key));
    }
    /**
    ...
    public function addService(string $name, object $service)
    {
        ...
        if (isset($this->instances[$name])) {
            throw new \Packetery\Nette\InvalidStateException(\sprintf("Service '%s' already exists.", $name));
        }
        if ($service instanceof \Closure) {
            $rt = \Packetery\Nette\Utils\Type::fromReflection(new \ReflectionFunction($service));
        ...
        if (!isset($this->methods[self::getMethodName($name)])) {
            ...
            throw new \Packetery\Nette\InvalidArgumentException(\sprintf("Service '%s' must be instance of %s, %s.", $name, $expectedType, $type ? "{$type} given" : 'add typehint to closure'));
        }
        if ($service instanceof \Closure) {
    ...
    public function createService(string $name) : object
    {
        ...
        $service = $this->preventDeadLock($name, function () use($callback, $method) {
            return $callback instanceof \Closure ? $callback() : $this->{$method}();
        });
        if (!\is_object($service)) {
            throw new \Packetery\Nette\UnexpectedValueException(\sprintf("Unable to create service '{$name}', value returned by %s is not object.", $callback instanceof \Closure ? 'closure' : "method {$method}()"));
        }
        return $service;
    }
    /**
    ...
    private function preventDeadLock(string $key, \Closure $callback)
    {
        if (isset($this->creating[$key])) {
            throw new \Packetery\Nette\InvalidStateException(\sprintf('Circular reference detected for: %s.', \implode(', ', \array_keys($this->creating))));
        }
        try {
    ...
    {
        return $function(...$this->autowireArguments(\Packetery\Nette\Utils\Callback::toReflection($function), $args));
    }
    ...
    {
        if ($name === '') {
            throw new \Packetery\Nette\InvalidArgumentException('Service name must be a non-empty string.');
        }
