...
use Packetery\Nette\PhpGenerator as Php;
...
final class FactoryDefinition extends Definition
{
    ...
    public function setImplement(string $interface)
    {
        ...
        if (!$method || $method->isStatic() || $method->name !== self::MethodCreate || \count($rc->getMethods()) > 1) {
            throw new \Packetery\Nette\InvalidArgumentException(\sprintf("Service '%s': Interface %s must have just one non-static method create().", $this->getName(), $interface));
        }
        try {
        ...
        if ($resultDef instanceof ServiceDefinition) {
            ...
            if ($resultDef->getEntity() instanceof Reference && !$resultDef->getCreator()->arguments) {
                $resultDef->setCreator([
                    ...
                    new Reference(\Packetery\Nette\DI\ContainerBuilder::ThisContainer),
    ...
    private function completeParameters(\Packetery\Nette\DI\Resolver $resolver) : void
    {
        ...
        foreach ($method->getParameters() as $param) {
            ...
            if (isset($ctorParams[$param->name])) {
                ...
                $hint = \Packetery\Nette\Utils\Helpers::getSuggestion($ctorParamsKeys, $param->name);
    ...
    public function generateMethod(Php\Method $method, \Packetery\Nette\DI\PhpGenerator $generator) : void
    {
