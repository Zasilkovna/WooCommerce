...
class DefinitionSchema implements Schema
{
    ...
    /** @var \Packetery\Nette\DI\ContainerBuilder */
    private $builder;
    public function __construct(\Packetery\Nette\DI\ContainerBuilder $builder)
    {
    ...
    public function merge($def, $base)
    {
        ...
        return \Packetery\Nette\Schema\Helpers::merge($def, $base);
    }
    /**
    ...
    public function normalize($def, Context $context)
    {
        if ($def === null || $def === \false) {
            ...
            if (isset($def['class']) && !isset($def['type']) && !isset($def['create']) && !isset($def['dynamic']) && !isset($def['imported'])) {
    ...
    private function expandParameters(array $config) : array
    {
        ...
        return \Packetery\Nette\DI\Helpers::expand($config, $params);
    }
