...
final class InjectExtension extends DI\CompilerExtension
{
            ...
            if ($hasAttr || DI\Helpers::parseAnnotation($rp, 'inject') !== null) {
                if ($type = Reflection::getPropertyType($rp)) {
                    ...
                    if (\strpos($type, '|') !== \false) {
                        throw new \Packetery\Nette\InvalidStateException('The ' . Reflection::toString($rp) . ' is not expected to have a union type.');
                    }
    ...
    private static function checkType($class, string $name, ?string $type, $container) : void
    {
        ...
        if (!$type) {
            throw new \Packetery\Nette\InvalidStateException("Property {$propName} has no type hint.");
        } elseif (!\class_exists($type) && !\interface_exists($type)) {
            throw new \Packetery\Nette\InvalidStateException("Class or interface '{$type}' used in type hint at {$propName} not found. Check type and 'use' statements.");
        } elseif ($container && !$container->getByType($type, \false)) {
            throw new \Packetery\Nette\DI\MissingServiceException("Service of type {$type} used in type hint at {$propName} not found. Did you add it to configuration file?");
        }
    }
}
