...
class UserStorage implements \Packetery\Nette\Security\IUserStorage
{
    ...
    public function setExpiration(?string $time, int $flags = 0)
    {
        $section = $this->getSessionSection(\true);
        if ($time) {
            $time = \Packetery\Nette\Utils\DateTime::from($time)->format('U');
            $section->expireTime = $time;
            $section->expireDelta = $time - \time();
        } else {
    ...
    protected function getSessionSection(bool $need) : ?SessionSection
    {
        ...
        if ($section->authenticated && $section->expireDelta > 0) {
            ...
            $section->expireTime = \time() + $section->expireDelta;
            // sliding expiration
        }
