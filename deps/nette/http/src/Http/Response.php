...
final class Response implements IResponse
{
     ...
     * @return static
     * @throws \Packetery\Nette\InvalidArgumentException  if code is invalid
     * @throws \Packetery\Nette\InvalidStateException  if HTTP headers have been sent
     */
    public function setCode(int $code, ?string $reason = null)
    {
        if ($code < 100 || $code > 599) {
            throw new \Packetery\Nette\InvalidArgumentException("Bad HTTP response '{$code}'.");
        }
        self::checkHeaders();
        $this->code = $code;
    ...
    public function setExpiration(?string $expire)
    {
        ...
        $this->setHeader('Cache-Control', 'max-age=' . ($expire->format('U') - \time()));
    ...
    private function checkHeaders() : void
    {
        if (\PHP_SAPI === 'cli') {
            ...
            throw new \Packetery\Nette\InvalidStateException('Cannot send header after HTTP headers have been sent' . ($file ? " (output started at {$file}:{$line})." : '.'));
        } elseif ($this->warnOnBuffer && \ob_get_length() && !\array_filter(\ob_get_status(\true), function (array $i) : bool {
            return !$i['chunk_size'];
        })) {
            \trigger_error('Possible problem: you are sending a HTTP header while already having some data in output buffer. Try Tracy\\OutputDebugger or send cookies/start session earlier.');
        }
    }
}
