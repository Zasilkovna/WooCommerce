...
class Session
{
    ...
    private const DefaultFileLifetime = 3 * \Packetery\Nette\Utils\DateTime::HOUR;
    ...
    private function doStart($mustExists = \false) : void
    {
        ...
        try {
            ...
            \Packetery\Nette\Utils\Callback::invokeSafe('session_start', [['read_and_close' => $this->readAndClose]], function (string $message) use(&$e) : void {
                $e = new \Packetery\Nette\InvalidStateException($message);
            });
        } catch (\Throwable $e) {
        }
        if ($e) {
        ...
        $this->initialize();
        \Packetery\Nette\Utils\Arrays::invoke($this->onStart, $this);
    }
    /** @internal */
    public function autoStart(bool $forWrite) : void
    {
        if ($this->started || !$forWrite && !$this->exists()) {
            ...
            \trigger_error('Cannot auto-start session because autostarting is disabled', \E_USER_WARNING);
            return;
        }
    ...
    {
        if (\session_status() !== \PHP_SESSION_ACTIVE) {
            throw new \Packetery\Nette\InvalidStateException('Session is not started.');
        }
        \session_destroy();
        $_SESSION = null;
        $this->started = \false;
    ...
    public function regenerateId() : void
    {
        ...
        if (\session_status() === \PHP_SESSION_ACTIVE) {
            if (\headers_sent($file, $line)) {
                throw new \Packetery\Nette\InvalidStateException('Cannot regenerate session ID after HTTP headers have been sent' . ($file ? " (output started at {$file}:{$line})." : '.'));
            }
    ...
    public function setName(string $name)
    {
        if (!\preg_match('#[^0-9.][^.]*$#DA', $name)) {
            throw new \Packetery\Nette\InvalidArgumentException('Session name cannot contain dot.');
        }
        \session_name($name);
    ...
    private function clean() : void
    {
        ...
        \Packetery\Nette\Utils\Arrays::invoke($this->onBeforeWrite, $this);
        $nf =& $_SESSION['__NF'];
     ...
     * @return static
     * @throws \Packetery\Nette\NotSupportedException
     * @throws \Packetery\Nette\InvalidStateException
     */
    public function setOptions(array $options)
    {
        ...
        foreach ($options as $key => $value) {
            ...
            if (!isset($allowed["session.{$normKey}"])) {
                $hint = \substr((string) \Packetery\Nette\Utils\Helpers::getSuggestion(\array_keys($allowed), "session.{$normKey}"), 8);
                if ($key !== $normKey) {
                ...
                throw new \Packetery\Nette\InvalidStateException("Invalid session configuration option '{$key}'" . ($hint ? ", did you mean '{$hint}'?" : '.'));
            }
            $normalized[$normKey] = $value;
        }
        if (isset($normalized['read_and_close'])) {
            if (\session_status() === \PHP_SESSION_ACTIVE) {
                throw new \Packetery\Nette\InvalidStateException('Cannot configure "read_and_close" for already started session.');
            }
    ...
    private function configure(array $config) : void
    {
        ...
        foreach ($config as $key => $value) {
            if ($value === null || \ini_get("session.{$key}") == $value) {
                ...
                if (\session_status() === \PHP_SESSION_ACTIVE) {
                    throw new \Packetery\Nette\InvalidStateException("Unable to set 'session.{$key}' to value '{$value}' when session has been started" . ($this->started ? '.' : ' by session.auto_start or session_start().'));
                }
                if (isset($special[$key])) {
                    ...
                    throw new \Packetery\Nette\NotSupportedException("Unable to set 'session.{$key}' to '{$value}' because function ini_set() is disabled.");
                }
            }
        }
    ...
    public function setExpiration(?string $expire)
    {
        if ($expire === null) {
            ...
            $expire = \Packetery\Nette\Utils\DateTime::from($expire)->format('U') - \time();
    ...
    {
        if ($this->started) {
            throw new \Packetery\Nette\InvalidStateException('Unable to set handler when session has been started.');
        }
    ...
    private function sendCookie() : void
    {
        ...
        $this->response->setCookie(\session_name(), \session_id(), $cookie['lifetime'] ? $cookie['lifetime'] + \time() : 0, $cookie['path'], $cookie['domain'], $cookie['secure'], $cookie['httponly'], $cookie['samesite'] ?? null);
    }
}
