...
class HttpExtension extends \Packetery\Nette\DI\CompilerExtension
{
    ...
    public function loadConfiguration()
    {
        ...
        $config = $this->config;
        $builder->addDefinition($this->prefix('requestFactory'))->setFactory(\Packetery\Nette\Http\RequestFactory::class)->addSetup('setProxy', [$config->proxy]);
        $request = $builder->addDefinition($this->prefix('request'))->setFactory('@\\Packetery\\Nette\\Http\\RequestFactory::fromGlobals');
        $response = $builder->addDefinition($this->prefix('response'))->setFactory(\Packetery\Nette\Http\Response::class);
    ...
    private function sendHeaders()
    {
        ...
        foreach (['csp', 'cspReportOnly'] as $key) {
            ...
            if (\strpos($value, "'nonce'")) {
                ...
                $value = \Packetery\Nette\DI\ContainerBuilder::literal('str_replace(?, ? . $cspNonce, ?)', ["'nonce", "'nonce-", $value]);
            }
        ...
        if (!$config->disableNetteCookie) {
            $this->initialization->addBody('\\Packetery\\Nette\\Http\\Helpers::initCookie($this->getService(?), $response);', [$this->prefix('request')]);
        }
    }
