...
```php
$class = new \Packetery\Nette\PhpGenerator\ClassType('Demo');
...
$class
	...
	->addComment('@property-read \Packetery\Nette\Forms\Form $form');
 ...
 * Second line
 *
 * @property-read \Packetery\Nette\Forms\Form $form
 */
...
```php
$printer = new \Packetery\Nette\PhpGenerator\Printer;
...
```php
$method = new \Packetery\Nette\PhpGenerator\Method('getHandle');
$property = new \Packetery\Nette\PhpGenerator\Property('handle');
$const = new \Packetery\Nette\PhpGenerator\Constant('ROLE');
...
```php
use \Packetery\Nette\PhpGenerator\Type;
...
$printer = new \Packetery\Nette\PhpGenerator\PsrPrinter;
...
```php
$interface = \Packetery\Nette\PhpGenerator\ClassType::interface('MyInterface');
$trait = \Packetery\Nette\PhpGenerator\ClassType::trait('MyTrait');
// in a similar way $class = \Packetery\Nette\PhpGenerator\ClassType::class('MyClass');
```
...
```php
$enum = \Packetery\Nette\PhpGenerator\ClassType::enum('Suit');
$enum->addCase('Clubs');
...
```php
use \Packetery\Nette\PhpGenerator\Literal;
...
```php
$class = new \Packetery\Nette\PhpGenerator\ClassType(null);
...
```php
$function = new \Packetery\Nette\PhpGenerator\GlobalFunction('foo');
...
// echo (new \Packetery\Nette\PhpGenerator\PsrPrinter)->printFunction($function);
```
...
```php
$closure = new \Packetery\Nette\PhpGenerator\Closure;
...
// or use PsrPrinter for output compatible with PSR-2 / PSR-12
// echo (new \Packetery\Nette\PhpGenerator\PsrPrinter)->printClosure($closure);
```
...
// or use PsrPrinter for output compatible with PSR-2 / PSR-12
echo (new \Packetery\Nette\PhpGenerator\Printer)->printArrowFunction($closure);
```
...
```php
$namespace = new \Packetery\Nette\PhpGenerator\PhpNamespace('Foo');
...
$class = new \Packetery\Nette\PhpGenerator\ClassType('Task');
$namespace->add($class);
```
...
// or use PsrPrinter for output compatible with PSR-2 / PSR-12
// echo (new \Packetery\Nette\PhpGenerator\PsrPrinter)->printNamespace($namespace);
```
...
```php
$printer = new \Packetery\Nette\PhpGenerator\Printer; // or PsrPrinter
...
```php
$file = new \Packetery\Nette\PhpGenerator\PhpFile;
...
// or use PsrPrinter for output compatible with PSR-2 / PSR-12
// echo (new \Packetery\Nette\PhpGenerator\PsrPrinter)->printFile($file);
```
...
```php
$class = \Packetery\Nette\PhpGenerator\ClassType::from(PDO::class);

$function = \Packetery\Nette\PhpGenerator\GlobalFunction::from('trim');

$closure = \Packetery\Nette\PhpGenerator\Closure::from(
...
```php
$class = \Packetery\Nette\PhpGenerator\ClassType::withBodiesFrom(MyClass::class);

$function = \Packetery\Nette\PhpGenerator\GlobalFunction::withBodyFrom('dump');
```
...
```php
$class = \Packetery\Nette\PhpGenerator\ClassType::fromCode(<<<XX
	<?php
...
```php
$file = \Packetery\Nette\PhpGenerator\PhpFile::fromCode(file_get_contents('classes.php'));
```
...
```php
$dumper = new \Packetery\Nette\PhpGenerator\Dumper;
...
```php
class MyPrinter extends \Packetery\Nette\PhpGenerator\Printer
{
