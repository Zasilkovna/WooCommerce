...
class SharedEventManager implements SharedEventManagerInterface
{
    ...
    public function attach($identifier, $event, callable $listener, $priority = 1)
    {
        if (!is_string($identifier) || empty($identifier)) {
            throw new Exception\InvalidArgumentException(sprintf('Invalid identifier provided; must be a string; received "%s"', is_object($identifier) ? $identifier::class : gettype($identifier)));
        }
        if (!is_string($event) || empty($event)) {
            throw new Exception\InvalidArgumentException(sprintf('Invalid event provided; must be a non-empty string; received "%s"', is_object($event) ? $event::class : gettype($event)));
        }
    ...
    public function detach(callable $listener, $identifier = null, $eventName = null, $force = \false)
    {
        ...
        if (!is_string($eventName) || empty($eventName)) {
            throw new Exception\InvalidArgumentException(sprintf('Invalid event name provided; must be a string, received %s', is_object($eventName) ? $eventName::class : gettype($eventName)));
        }
