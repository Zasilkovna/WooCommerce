...
use Packetery\Laminas\Code\Annotation\AnnotationManager;
use Packetery\Laminas\Code\Scanner\AnnotationScanner;
use Packetery\Laminas\Code\Scanner\CachingFileScanner;
...
class MethodReflection extends PhpReflectionMethod implements ReflectionInterface
{
        ...
        if ($format == MethodReflection::PROTOTYPE_AS_STRING) {
            ...
            foreach ($prototype['arguments'] as $name => $argument) {
                ...
                if (!$argument['required']) {
                    $argsLine .= ' = ' . var_export($argument['default'], \true);
                }
                $args[] = $argsLine;
            }
            $line .= implode(', ', $args);
    ...
    {
        return $this->extractMethodContents(\true);
    }
    /**
    ...
    protected function extractMethodContents($bodyOnly = \false)
    {
        ...
        $lines = array_slice(file($fileName, \FILE_IGNORE_NEW_LINES), $this->getStartLine() - 1, $this->getEndLine() - ($this->getStartLine() - 1), \true);
        ...
        foreach ($tokens as $key => $token) {
            ...
            switch ($tokenType) {
                ...
                default:
                    ...
                    if ($bodyOnly === \true && $firstBrace !== \true) {
                        break;
                    }
                    $body .= $tokenValue;
                    break;
