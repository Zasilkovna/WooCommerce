 ...
 */
namespace Packetery\Laminas\Code\Scanner;

use Packetery\Laminas\Code\Annotation;
use Packetery\Laminas\Code\Exception;
use Packetery\Laminas\Code\NameInformation;
use ReflectionClass;
...
class ClassScanner implements ScannerInterface
{
     ...
     */
    public function getConstants($namesOnly = \true)
    {
        if (\true === $namesOnly) {
    ...
    protected function scan()
    {
        ...
        $MACRO_TOKEN_ADVANCE = function () use(&$tokens, &$tokenIndex, &$token, &$tokenType, &$tokenContent, &$tokenLine) {
            ...
            if (is_string($token)) {
                $tokenType = null;
                $tokenContent = $token;
                $tokenLine += substr_count($lastTokenArray[1] ?? '', "\n");
                ...
                case \T_USE:
                    ...
                    if ($tokenType !== null) {
                        ...
                        if (\false === $useAliasContext) {
                            ...
                            // in alias context
                            if ($useAsContext === \true && in_array($tokenType, $isAlias)) {
