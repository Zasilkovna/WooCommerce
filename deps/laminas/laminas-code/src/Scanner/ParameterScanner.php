...
class ParameterScanner
{
    ...
    protected function scan()
    {
        ...
        if (is_string($token)) {
            // check pass by ref
            if ($token === '&') {
                $this->isPassedByReference = \true;
                goto SCANNER_CONTINUE;
            }
            if ($token === '=') {
                $this->isOptional = \true;
                $this->isDefaultValueAvailable = \true;
                goto SCANNER_CONTINUE;
            }
        } else {
            if ($this->name === null && ($token[0] === \T_STRING || $token[0] === \T_NS_SEPARATOR || $token[0] === \T_NAME_QUALIFIED || $token[0] === \T_NAME_FULLY_QUALIFIED)) {
                $this->class .= $token[1];
                goto SCANNER_CONTINUE;
            }
