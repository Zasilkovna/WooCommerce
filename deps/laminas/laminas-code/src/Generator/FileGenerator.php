 ...
 */
namespace Packetery\Laminas\Code\Generator;

use Packetery\Laminas\Code\DeclareStatement;
use Packetery\Laminas\Code\Exception\InvalidArgumentException;
use Packetery\Laminas\Code\Generator\Exception\ClassNotFoundException;
use Packetery\Laminas\Code\Reflection\Exception as ReflectionException;
use Packetery\Laminas\Code\Reflection\FileReflection;
...
class FileGenerator extends AbstractGenerator
{
     ...
     */
    public static function fromReflectedFileName($filePath, $includeIfNotAlreadyIncluded = \true)
    {
    ...
    public function generate()
    {
        ...
        if ($this->declares) {
            ...
            if (preg_match('#/\\* Laminas_Code_Generator_FileGenerator-DeclaresMarker \\*/#m', $output)) {
                $output = preg_replace('#/\\* Laminas_Code_Generator_FileGenerator-DeclaresMarker \\*/#m', $declareStatements, $output, 1);
            } else {
        ...
        if (!empty($uses)) {
            ...
            if (preg_match('#/\\* Laminas_Code_Generator_FileGenerator-UseMarker \\*/#m', $output)) {
                $output = preg_replace('#/\\* Laminas_Code_Generator_FileGenerator-UseMarker \\*/#m', $useOutput, $output, 1);
            } else {
                $output .= $useOutput;
            }
        }
