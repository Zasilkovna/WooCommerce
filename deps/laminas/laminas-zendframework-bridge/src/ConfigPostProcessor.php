...
class ConfigPostProcessor
{
    ...
    public function __construct()
    {
        ...
        $this->rulesets = [
            ...
            // Array values
            function ($value, array $keys) {
                return $keys !== [] && is_array($value) ? [$this, 'processConfig'] : null;
            },
        ];
    }
    /**
    ...
    private function initializeReplacements(array $config) : Replacements
    {
        ...
        if (!is_array($replacements)) {
            throw new RuntimeException(\sprintf('Invalid laminas-zendframework-bridge.replacements configuration;' . ' value MUST be an array; received %s', \is_object($replacements) ? \get_class($replacements) : \gettype($replacements)));
        }
        foreach ($replacements as $lookup => $replacement) {
            if (!is_string($lookup) || !is_string($replacement) || \preg_match('/^\\s*$/', $lookup) || \preg_match('/^\\s*$/', $replacement)) {
                throw new RuntimeException('Invalid lookup or replacement in laminas-zendframework-bridge.replacements configuration;' . ' all keys and values MUST be non-empty strings.');
            }
        }
