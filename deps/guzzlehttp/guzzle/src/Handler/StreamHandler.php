...
use Packetery\GuzzleHttp\Promise\PromiseInterface;
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \Packetery\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \Packetery\GuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \Packetery\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \Packetery\GuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\Packetery\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \Packetery\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \Packetery\GuzzleHttp\debug_resource($value);
