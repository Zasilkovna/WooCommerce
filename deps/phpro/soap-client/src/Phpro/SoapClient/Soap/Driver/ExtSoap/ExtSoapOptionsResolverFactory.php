...
use Packetery\Phpro\SoapClient\Soap\TypeConverter\TypeConverterCollection;
use Packetery\Phpro\SoapClient\Soap\TypeConverter\TypeConverterInterface;
use Packetery\Symfony\Component\OptionsResolver\Options;
...
class ExtSoapOptionsResolverFactory
{
    ...
    public static function create() : OptionsResolver
    {
        ...
        $resolver->setAllowedValues('compression', function ($value) : bool {
            ...
            return $value >= 0 && $value <= (\SOAP_COMPRESSION_ACCEPT | \SOAP_COMPRESSION_DEFLATE | \SOAP_COMPRESSION_GZIP | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 | 9);
        });
        // Encoding
        $resolver->setDefined(['encoding']);
        $resolver->setAllowedTypes('encoding', ['string']);
        // Trace
        $resolver->setDefault('trace', \true);
        ...
        $resolver->setNormalizer('classmap', function (Options $options, $value) : array {
            ...
            return \array_map(function (ClassMap $classMap) {
                return $classMap->getPhpClassName();
            }, \iterator_to_array($value));
        });
        // Exceptions
        $resolver->setDefault('exceptions', \true);
        ...
        $resolver->setNormalizer('typemap', function (Options $options, $value) : array {
            ...
            return \array_values(\array_map(function (TypeConverterInterface $converter) {
                return ['type_name' => $converter->getTypeName(), 'type_ns' => $converter->getTypeNamespace(), 'from_xml' => function ($input) use($converter) {
        ...
        $resolver->setAllowedValues('cache_wsdl', [\WSDL_CACHE_NONE, \WSDL_CACHE_DISK, \WSDL_CACHE_MEMORY, \WSDL_CACHE_BOTH]);
        // User agent
        ...
        $resolver->setAllowedValues('features', function ($value) : bool {
            return $value >= 0 && $value <= (\SOAP_SINGLE_ELEMENT_ARRAYS | \SOAP_USE_XSI_ARRAY_TYPE | \SOAP_WAIT_ONE_WAY_CALLS);
        });
        // Keep alive
        ...
        $resolver->setAllowedValues('ssl_method', [\SOAP_SSL_METHOD_TLS, \SOAP_SSL_METHOD_SSLv2, \SOAP_SSL_METHOD_SSLv3, \SOAP_SSL_METHOD_SSLv23]);
        return $resolver;
    }
}
