...
class MethodsParser
{
    ...
    {
        return new MethodCollection(...\array_map(function (string $methodString) {
    ...
    private function parseMethodFromString(string $methodString) : Method
    {
        ...
        return new Method($this->parseName($methodString), $this->parseParameters($methodString), $this->parseReturnType($methodString));
    }
    ...
    private function parseParameters(string $methodString) : array
    {
        ...
        return \array_map(function (string $parameter) : Parameter {
            list($type, $name) = \explode(' ', \trim($parameter));
            return new Parameter(\ltrim($name, '$'), $this->xsdTypes->fetchByNameWithFallback($type));
        }, $parameters);
    }
