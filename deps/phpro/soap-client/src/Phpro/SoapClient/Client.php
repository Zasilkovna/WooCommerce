...
use Packetery\Phpro\SoapClient\Event;
...
use Packetery\Phpro\SoapClient\Type\ResultProviderInterface;
use Packetery\Phpro\SoapClient\Util\XmlFormatter;
use Packetery\Symfony\Component\EventDispatcher\EventDispatcherInterface as SymfonyEventDispatcher;
/**
 * Class Client
 *
...
class Client implements ClientInterface
{
    ...
    {
        \assert($dispatcher instanceof SymfonyEventDispatcher || $dispatcher instanceof EventDispatcherInterface, new RuntimeException(\sprintf('Expected event dispatcher to be of type %s or %s, got "%s".', SymfonyEventDispatcher::class, EventDispatcherInterface::class, \get_class($dispatcher))));
        $this->engine = $engine;
    ...
    public function debugLastSoapRequest() : array
    {
        ...
        return ['request' => ['headers' => \trim($lastRequestInfo->getLastRequestHeaders()), 'body' => XmlFormatter::format($lastRequestInfo->getLastRequest())], 'response' => ['headers' => \trim($lastRequestInfo->getLastResponseHeaders()), 'body' => XmlFormatter::format($lastRequestInfo->getLastResponse())]];
    }
    /**
    ...
    {
        $dispatcher = $this->dispatcher instanceof EventDispatcherInterface ? $this->dispatcher : new Event\Dispatcher\SymfonyEventDispatcher($this->dispatcher);
