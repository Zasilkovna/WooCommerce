...
use Packetery\Phpro\SoapClient\Soap\Engine\Metadata\Model\Method as MetadataMethod;
use Packetery\Phpro\SoapClient\Soap\Engine\Metadata\Model\Parameter as MetadataParameter;
/**
 * Class ClientMethod
 *
...
class ClientMethod
{
    ...
    public static function fromMetadata(string $parameterNamespace, MetadataMethod $method) : self
    {
        return new self($method->getName(), \array_map(function (MetadataParameter $parameter) use($parameterNamespace) {
            return Parameter::fromMetadata($parameterNamespace, $parameter);
        }, $method->getParameters()), $method->getReturnType()->getBaseTypeOrFallbackToName(), $parameterNamespace);
    }
    /**
