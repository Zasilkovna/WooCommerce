...
class Normalizer
{
    private static $normalizations = ['any' => 'mixed', 'anytype' => 'mixed', 'long' => 'int', 'short' => 'int', 'datetime' => '\\DateTimeInterface', 'date' => '\\DateTimeInterface', 'boolean' => 'bool', 'decimal' => 'float', 'double' => 'float', 'integer' => 'int', 'string' => 'string', 'self' => 'self', 'callable' => 'callable', 'iterable' => 'iterable', 'array' => 'array'];
    /**
     * @var array
    ...
    {
        if (!\in_array(\strtolower($name), self::$reservedKeywords, \true)) {
            return $name;
        }
        return $name . $suffix;
    }
    ...
    {
        return \trim(\str_replace('/', '\\', $namespace), '\\');
    }
    /**
    ...
    {
        return \in_array($type, self::$normalizations, \true);
    }
    /**
     * @param string $prefix
     * @param string $property
