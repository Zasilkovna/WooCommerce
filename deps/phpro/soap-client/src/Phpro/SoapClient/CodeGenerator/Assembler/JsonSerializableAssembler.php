...
class JsonSerializableAssembler implements AssemblerInterface
{
    ...
    private function implementJsonSerialize(Type $type, ClassGenerator $class)
    {
        ...
        $class->addMethodFromGenerator(MethodGenerator::fromArray(['name' => $methodName, 'parameters' => [], 'visibility' => MethodGenerator::VISIBILITY_PUBLIC, 'body' => $this->generateJsonSerializeBody($type, $class), 'docblock' => DocBlockGeneratorFactory::fromArray(['tags' => [['name' => 'return', 'description' => 'array']]])]));
    }
    /**
     * @param Type $type
     * @param ClassGenerator $class
