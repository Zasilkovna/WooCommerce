...
class FluentSetterAssembler implements AssemblerInterface
{
    ...
    public function assemble(ContextInterface $context)
    {
        ...
        try {
            ...
            $methodGenerator->setBody(\sprintf('$this->%1$s = $%1$s;%2$sreturn $this;', $property->getName(), $class::LINE_FEED));
            if ($this->options->useReturnType()) {
                $methodGenerator->setReturnType($class->getNamespaceName() . '\\' . $class->getName());
            }
            if ($this->options->useDocBlocks()) {
                $methodGenerator->setDocBlock(DocBlockGeneratorFactory::fromArray(['tags' => [['name' => 'param', 'description' => \sprintf('%s $%s', $property->getType(), $property->getName())], ['name' => 'return', 'description' => '$this']]]));
            }
