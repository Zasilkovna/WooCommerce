...
class GenerateClassmapCommand extends Command
{
    ...
    {
        $this->setName(self::COMMAND_NAME)->setDescription('Generates a classmap based on WSDL.')->addOption('config', null, InputOption::VALUE_REQUIRED, 'The location of the soap code-generator config file')->addOption('overwrite', 'o', InputOption::VALUE_NONE, 'Makes it possible to overwrite by default');
    }
    /**
     * {@inheritdoc}
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $typeMap = TypeMap::fromMetadata($config->getTypeNamespace(), $config->getEngine()->getMetadata()->getTypes());
        $generator = new ClassMapGenerator($config->getRuleSet(), $config->getClassMapName(), $config->getClassMapNamespace());
     ...
     * @param string $path
     */
    protected function generateClassmap(FileGenerator $file, ClassMapGenerator $generator, TypeMap $typeMap, string $path)
    {
