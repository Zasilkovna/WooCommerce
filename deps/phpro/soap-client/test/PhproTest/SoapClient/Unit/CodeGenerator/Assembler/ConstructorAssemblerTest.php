...
use Packetery\Phpro\SoapClient\CodeGenerator\Assembler\ConstructorAssembler;
use Packetery\Phpro\SoapClient\CodeGenerator\Assembler\ConstructorAssemblerOptions;
...
CODE;
    ...
    function it_assembles_a_type_with_type_hints()
    {
        ...
        $type = new Type($namespace = 'MyNamespace', 'MyType', [new Property('prop1', 'string', $namespace), new Property('prop2', 'int', $namespace), new Property('prop3', 'SomeClass', $namespace)]);
...
CODE;
    ...
    {
        $assembler = new ConstructorAssembler((new ConstructorAssemblerOptions())->withDocBlocks(\false)->withTypeHints(\true));
        $context = $this->createContext();
        $assembler->assemble($context);
        $code = $context->getClass()->generate();
        $expected = <<<CODE
