...
use Packetery\Phpro\SoapClient\CodeGenerator\Assembler\ClientMethodAssembler;
...
class ClientMethodAssemblerTest extends TestCase
{
    ...
    private function createContext()
    {
        ...
        $class->setNamespaceName($namespace = 'Packetery\\Vendor\\MyNamespace');
        $typeNamespace = 'Packetery\\Vendor\\MyTypeNamespace';
        $method = new ClientMethod('functionName', [new Parameter('param', $typeNamespace . '\\ParamType')], 'ReturnType', $typeNamespace);
    ...
    private function createMultiArgumentContext()
    {
        ...
        $class->setNamespaceName($namespace = 'Packetery\\Vendor\\MyNamespace');
        $typeNamespace = 'Packetery\\Vendor\\MyTypeNamespace';
        $method = new ClientMethod('functionName', [new Parameter('param', $typeNamespace . '\\ParamType'), new Parameter('param2', $typeNamespace . '\\OtherParamType')], 'ReturnType', $typeNamespace);
        return new ClientMethodContext($class, $method);
    }
    /**
     * @test
...
CODE;
    ...
    function it_assembles_a_method_with_underscore_param_type()
    {
        $assembler = new ClientMethodAssembler();
        $class = new ClassGenerator();
        $class->setNamespaceName('Packetery\\Vendor\\MyNamespace');
        $typeNamespace = 'Packetery\\Vendor\\MyTypeNamespace';
        $method = new ClientMethod('Function_name', [new Parameter('param', $typeNamespace . '\\param_type')], 'return_type', $typeNamespace);
...
CODE;
    ...
    function it_throws_an_exception_when_wrong_context_is_passed()
    {
        ...
        $this->expectExceptionMessage(\sprintf('Phpro\\SoapClient\\CodeGenerator\\Assembler\\ClientMethodAssembler::assemble ' . 'expects an Phpro\\SoapClient\\CodeGenerator\\Context\\ClientMethodContext as input %s given', \get_class($context)));
