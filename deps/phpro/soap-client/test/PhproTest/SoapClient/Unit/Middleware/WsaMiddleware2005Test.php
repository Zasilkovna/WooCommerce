...
use Packetery\Phpro\SoapClient\Middleware\WsaMiddleware2005;
...
class WsaMiddleware2005Test extends TestCase
{
    ...
    function it_adds_wsa_to_the_request_xml()
    {
        ...
        $result = $this->client->sendRequest($request = new Request('POST', '/endpoint', ['SOAPAction' => 'myaction'], $soapRequest));
        ...
        $this->assertMatchesRegularExpression('/^uuid:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i', $xml->xpath('//soap:Header/wsa:MessageID')->item(0)->nodeValue);
        $this->assertEquals(WsaMiddleware2005::WSA_ADDRESS2005_ANONYMOUS, $xml->xpath('//soap:Header/wsa:ReplyTo/wsa:Address')->item(0)->nodeValue);
    }
    /**
     * @param $soapBody
     *
