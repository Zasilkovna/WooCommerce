...
use Packetery\Phpro\SoapClient\Middleware\WsseMiddleware;
...
use Packetery\RobRichards\XMLSecLibs\XMLSecurityKey;
/**
...
class WsseMiddlewareTest extends TestCase
{
    ...
    {
        $this->middleware = new WsseMiddleware(FIXTURE_DIR . '/certificates/wsse-client-private-key.pem', FIXTURE_DIR . '/certificates/wsse-client-public-key.pub');
    ...
    function it_adds_Wsse_to_the_request_xml()
    {
        ...
        // Check defaults:
        $this->assertEquals(XMLSecurityKey::RSA_SHA1, (string) $xml->xpath('//ds:SignatureMethod')->item(0)->getAttribute('Algorithm'));
        $this->assertEquals(\strtotime((string) $xml->xpath('//wsu:Created')->item(0)->nodeValue), \strtotime((string) $xml->xpath('//wsu:Expires')->item(0)->nodeValue) - 3600);
    }
    /**
     * @test
     */
    function it_is_possible_to_configure_expiry_ttl()
    {
        ...
        $soapBody = (string) $this->mockClient->getRequests()[0]->getBody();
        $xml = $this->fetchSoapXml($soapBody);
        $this->assertEquals(\strtotime((string) $xml->xpath('//wsu:Created')->item(0)->nodeValue), \strtotime((string) $xml->xpath('//wsu:Expires')->item(0)->nodeValue) - 100);
    }
    /**
     * @test
     */
    ...
    function it_is_possible_to_specify_another_digital_signature_method()
    {
        ...
        $xml = $this->fetchSoapXml($soapBody);
        // Check defaults:
        $this->assertEquals(XMLSecurityKey::RSA_SHA256, (string) $xml->xpath('//ds:SignatureMethod')->item(0)->getAttribute('Algorithm'));
    }
    /**
     * @test
     */
    function it_is_possible_to_specify_a_user_token()
    {
        ...
        $this->assertEquals('http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText', (string) $xml->xpath('//wsse:Password')->item(0)->getAttribute('Type'));
    }
    /**
     * @test
     */
    ...
    {
        $this->middleware->withUserToken('username', 'password', \true);
        $soapRequest = \file_get_contents(FIXTURE_DIR . '/soap/empty-request.xml');
        $this->mockClient->addResponse($response = new Response(200));
        $this->client->sendRequest($request = new Request('POST', '/', ['SOAPAction' => 'myaction'], $soapRequest));
        $soapBody = (string) $this->mockClient->getRequests()[0]->getBody();
        ...
        $this->assertEquals('http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest', (string) $xml->xpath('//wsse:Password')->item(0)->getAttribute('Type'));
    }
    /**
     * @test
     */
    function it_is_possible_to_encrypt_a_request()
    {
        ...
        $this->middleware->withServerCertificateHasSubjectKeyIdentifier(\true);
