...
class TypesManipulatorChainTest extends TestCase
{
    ...
    {
        $types = new TypeCollection();
        $chain = new TypesManipulatorChain(new class implements TypesManipulatorInterface
        {
            public function __invoke(TypeCollection $allTypes) : TypeCollection
            {
                return new TypeCollection(...\array_merge(\iterator_to_array($allTypes), [new Type(XsdType::create('Response'), [])]));
            }
        }, new class implements TypesManipulatorInterface
        {
            public function __invoke(TypeCollection $allTypes) : TypeCollection
            {
                return new TypeCollection(...\array_merge(\iterator_to_array($allTypes), [new Type(XsdType::create('Response2'), [])]));
            }
        });
        $result = $chain($types);
        ...
        self::assertEquals([new Type(XsdType::create('Response'), []), new Type(XsdType::create('Response2'), [])], \iterator_to_array($result));
    }
}
