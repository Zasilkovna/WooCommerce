...
class EnumTest extends AbstractSoapTestCase
{
    ...
    {
        $this->driver = $this->configureSoapDriver($this->wsdl, ['typemap' => [['type_name' => 'PhoneTypeEnum', 'type_ns' => 'http://soapinterop.org/xsd', 'from_xml' => function ($xml) {
            $doc = new \DOMDocument();
            $doc->loadXML($xml);
            if ('' === $doc->textContent) {
                return null;
    ...
    private function createEnum(string $value)
    {
        return new class($value)
        {
            ...
            {
                if (!\in_array($value, ['Home', 'Office', 'Gsm'], \true)) {
