...
namespace Packetery\PhproTest\SoapClient\Functional\ExtSoap\Encoding;
...
use Packetery\PhproTest\SoapClient\Functional\ExtSoap\AbstractSoapTestCase;
/** @internal */
class DuplicateTypenamesTest extends AbstractSoapTestCase
{
    ...
    protected function setUp() : void
    {
        ...
        $this->handler = $this->configureServer($this->wsdl, [], new class
        {
    ...
    {
        $this->driver = $this->configureSoapDriver($this->wsdl, ['classmap' => new ClassMapCollection([new ClassMap('Store', DuplicateTypeStore::class)])]);
        $this->handler = $this->configureServer($this->wsdl, [], new class
        {
            public function validate($store1, $store2)
            {
                return ['output1' => new DuplicateTypeStore('attr1', null), 'output2' => new DuplicateTypeStore(null, 'attr2')];
            }
        });
        $engine = new Engine($this->driver, $this->handler);
    ...
    {
        $this->driver = $this->configureSoapDriver($this->wsdl, ['classmap' => new ClassMapCollection([new ClassMap('Store', DuplicateTypeStore::class)]), 'typemap' => [['type_name' => 'Store', 'type_ns' => 'http://soapinterop.org/xsd1', 'from_xml' => function ($xml) {
            $doc = new \DOMDocument();
            $doc->loadXML($xml);
