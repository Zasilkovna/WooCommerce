...
use Packetery\Phpro\SoapClient\Exception\UnexpectedConfigurationException;
...
use Packetery\Phpro\SoapClient\Soap\Driver\ExtSoap\ExtSoapOptionsResolverFactory;
...
use Packetery\Phpro\SoapClient\Soap\TypeConverter;
...
use Packetery\Symfony\Component\OptionsResolver\OptionsResolver;
/** @internal */
class ExtSoapOptionsTest extends TestCase
{
    ...
    {
        $options = new ExtSoapOptions($this->wsdl, $expectedOptions = ['trace' => \true]);
    ...
    {
        $options = $this->resolver->resolve(ExtSoapOptions::defaults($this->wsdl, [])->getOptions());
    ...
    {
        $options = $this->resolver->resolve(ExtSoapOptions::defaults($this->wsdl, ['trace' => \false, 'proxy_host' => $proxyHost = 'http://localhost'])->getOptions());
    ...
    function it_is_possible_to_attach_a_wsdl_provider()
    {
        ...
        $options = ExtSoapOptions::defaults($this->wsdl, [])->withWsdlProvider($wsdlProvider->reveal());
        $this->assertSame($newWsdl, $options->getWsdl());
    }
    /** @test */
    function it_is_possible_to_disable_wsdl_cache()
    {
        $options = $this->resolver->resolve(ExtSoapOptions::defaults($this->wsdl)->disableWsdlCache()->getOptions());
    ...
    {
        $options = ExtSoapOptions::defaults($this->wsdl)->withTypeMap($typeMap = new TypeConverter\TypeConverterCollection());
    ...
    {
        $options = ExtSoapOptions::defaults($this->wsdl, ['typemap' => [['type_name' => $typeName = 'hello', 'type_ns' => $typeNs = 'http://my-ns/xsd', 'from_xml' => function ($input) {
            return $input;
    ...
    {
        $options = ExtSoapOptions::defaults($this->wsdl)->withClassMap($classMap = new ClassMapCollection([new ClassMap('wsdlType', 'PhpClass')]));
    ...
    {
        $options = ExtSoapOptions::defaults($this->wsdl, ['classmap' => ['wsdlType' => 'PhpClass']]);
        $resolved = $this->resolver->resolve($options->getOptions());
        $this->assertIsArray($resolved['classmap']);
        $this->assertCount(1, $resolved['classmap']);
        $this->assertSame('PhpClass', $resolved['classmap']['wsdlType']);
    ...
    public function it_adds_prefered_default_metadata_options() : void
    {
        $options = ExtSoapOptions::defaults($this->wsdl);
        $metadataOptions = $options->getMetadataOptions();
        $expectedMetadataOptions = MetadataOptions::empty()->withTypesManipulator(new TypesManipulatorChain(new IntersectDuplicateTypesStrategy()));
