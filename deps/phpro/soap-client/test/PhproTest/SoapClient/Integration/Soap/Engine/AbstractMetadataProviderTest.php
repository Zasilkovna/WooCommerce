...
{
    protected abstract function getMetadataProvider() : MetadataProviderInterface;
    /** @test */
    public function it_can_load_wsdl_methods()
    {
        ...
        $this->assertMethodExists($methods, 'validate', [new Parameter('input', XsdType::create('string'))], XsdType::create('string'));
    }
    /** @test */
    function it_can_load_wsdl_method_with_multiple_response_arguments()
    {
        ...
        $methods = $metadata->getMethods();
        $this->assertCount(1, $methods);
        $this->assertMethodExists($methods, 'validate', [new Parameter('input', XsdType::create('string'))], XsdType::create('array'));
    }
    /** @test */
    function it_can_load_union_types_in_methods()
    {
        ...
        $metadata = $this->getMetadataProvider()->getMetadata();
        $methods = $metadata->getMethods();
        $jeansType = XsdType::create('jeansSize')->withBaseType('anyType')->withMemberTypes(['sizebyno', 'sizebystring']);
        $this->assertCount(1, $methods);
        $this->assertMethodExists($methods, 'validate', [new Parameter('input', $jeansType)], $jeansType);
    }
    /** @test */
    function it_can_load_list_types_in_methods()
    {
        ...
        $metadata = $this->getMetadataProvider()->getMetadata();
        $methods = $metadata->getMethods();
        $listType = XsdType::create('valuelist')->withBaseType('array')->withMemberTypes(['integer']);
        $this->assertCount(1, $methods);
        $this->assertMethodExists($methods, 'validate', [new Parameter('input', $listType)], $listType);
    }
    /** @test */
    function it_can_load_simple_content_types()
    {
        ...
        $this->assertTypeExists($types, XsdType::create('SimpleContent'), [new Property('_', XsdType::create('integer')), new Property('country', XsdType::create('string'))]);
    }
    /** @test */
    function it_can_load_complex_types()
    {
        ...
        $this->assertTypeExists($types, XsdType::create('ValidateRequest'), [new Property('input', XsdType::create('string'))]);
        $this->assertTypeExists($types, XsdType::create('ValidateResponse'), [new Property('output', XsdType::create('string'))]);
    }
    /** @test */
    function it_can_load_union_types()
    {
        ...
        $jeansType = XsdType::create('jeansSize')->withBaseType('anyType')->withMemberTypes(['sizebyno', 'sizebystring']);
        $this->assertCount(1, $types);
        $this->assertTypeExists($types, XsdType::create('jeansSizeContainer'), [new Property('jeansSize', $jeansType)]);
    }
    /** @test */
    function it_can_load_list_types()
    {
        ...
        $listType = XsdType::create('valuelist')->withBaseType('array')->withMemberTypes(['integer']);
        $this->assertCount(1, $types);
        $this->assertTypeExists($types, XsdType::create('valuelistContainer'), [new Property('valuelist', $listType)]);
    }
    /** @test */
