...
use DateTime;
use Packetery\Phpro\SoapClient\Soap\TypeConverter\DateTimeTypeConverter;
...
class DateTimeTypeConverterSpec extends ObjectBehavior
{
    ...
    function it_returns_correct_datetime_string_on_valid_type_passed()
    {
        ...
        $this->convertPhpToXml($dateTime)->shouldReturn('<dateTime>' . $dateTime->format('Y-m-d\\TH:i:sP') . '</dateTime>');
    }
    public function getMatchers() : array
    {
        return ['matchWithCurrentTimeZone' => function (\DateTimeZone $dateTimeZone) {
            $name = $dateTimeZone->getName();
            return $name === \date('T') || $name === \date('P') || $name === \date('e');
        }];
    }
}
