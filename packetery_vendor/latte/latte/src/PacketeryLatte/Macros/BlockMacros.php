...
use PacketeryLatte\Runtime\Block;
use PacketeryLatte\Runtime\SnippetDriver;
...
class BlockMacros extends MacroSet
{
	...
	{
		//trigger_error('Macro {includeblock} is deprecated, use {include 'file.view' with blocks} or similar macro {import}.', E_USER_DEPRECATED);
		$node->replaced = false;
		$node->validate(true);
		return $writer->write(
			'ob_start(function () {});
	...
	public function macroBlock(MacroNode $node, PhpWriter $writer): string
	{
		...
		if (Helpers::startsWith((string) $node->context[1], PacketeryLatte\Compiler::CONTEXT_HTML_ATTRIBUTE)) {
			$node->context[1] = '';
			$node->modifiers .= '|escape';
	...
	public function macroIfset(MacroNode $node, PhpWriter $writer)
	{
		...
		while ([$name, $block] = $node->tokenizer->fetchWordWithModifier('block')) {
			$list[] = $block || preg_match('~#|[\w-]+$~DA', $name)
				...
				: 'isset(' . $writer->formatArgs(new PacketeryLatte\MacroTokens($name)) . ')';
		}
