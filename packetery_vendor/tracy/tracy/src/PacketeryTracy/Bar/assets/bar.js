...
class Panel
{
	constructor(id) {
		...
		this.elem.PacketeryTracy = this.elem.PacketeryTracy || {};
	}
	...
	init() {
		...
		PacketeryTracy.Dumper.init(Debug.layer);
		...
		elem.addEventListener('mouseenter', () => {
			clearTimeout(elem.PacketeryTracy.displayTimeout);
		});
		...
		if (this.is('tracy-panel-persist')) {
			PacketeryTracy.Toggle.persist(elem);
		}
	}
	...
	focus() {
		...
		if (this.is(Panel.WINDOW)) {
			elem.PacketeryTracy.window.focus();
	...
	blur() {
		...
		if (this.is(Panel.PEEK)) {
			clearTimeout(elem.PacketeryTracy.displayTimeout);
			elem.PacketeryTracy.displayTimeout = setTimeout(() => {
	...
	toWindow() {
		...
		+ '<script src="' + (baseUrl.replace(/&/g, '&amp;').replace(/"/g, '&quot;')) + '_tracy_bar=js&amp;XDEBUG_SESSION_STOP=1" onload="PacketeryTracy.Dumper.init()" async></script>'
		+ '<body id="tracy-debug">'
		);
		...
		this.elem.PacketeryTracy.window = win;
		return true;
	}
...
class Debug
{
	...
	static captureAjax() {
		let header = PacketeryTracy.getAjaxHeader();
		if (!header) {
			return;
		}
		...
		XMLHttpRequest.prototype.open = function() {
			...
			if (autoRefresh && new URL(arguments[1], location.origin).host === location.host) {
				...
				this.setRequestHeader('X-PacketeryTracy-Ajax', reqId);
				this.addEventListener('load', function() {
					if (this.getAllResponseHeaders().match(/^X-PacketeryTracy-Ajax: 1/mi)) {
		...
		window.fetch = function(request, options) {
			...
			if (autoRefresh && new URL(request.url, location.origin).host === location.host) {
				let reqId = header + '_' + ajaxCounter++;
				request.headers.set('X-PacketeryTracy-Ajax', reqId);
				return oldFetch(request).then((response) => {
					if (response instanceof Response && response.headers.has('X-PacketeryTracy-Ajax') && response.headers.get('X-PacketeryTracy-Ajax')[0] === '1') {
						Debug.loadScript(baseUrl + '_tracy_bar=content-ajax.' + reqId + '&XDEBUG_SESSION_STOP=1&v=' + Math.random());
					}
...
PacketeryTracy.DebugPanel = Panel;
PacketeryTracy.DebugBar = Bar;
PacketeryTracy.Debug = Debug;
PacketeryTracy.getAjaxHeader = () => contentId;

Debug.setOptions({
	panelZIndex: PacketeryTracy.panelZIndex,
	maxAjaxRows: window.PacketeryTracyMaxAjaxRows,
	autoRefresh: window.PacketeryTracyAutoRefresh,
});
