...
class Bar
{
	...
	{
		if (!$this->useSession) {
			throw new \LogicException('Start session before PacketeryTracy is enabled.');
		}
	...
	public function dispatchAssets(): bool
	{
		...
		if ($this->useSession && Helpers::isAjax()) {
			header('X-PacketeryTracy-Ajax: 1'); // session must be already locked
		}
		...
		if ($this->useSession && is_string($asset) && preg_match('#^content(-ajax)?\.(\w+)$#', $asset, $m)) {
			...
			if ($session) {
				...
				echo "PacketeryTracy.Debug.$method(", json_encode($session['content'], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE), ');';
				$session = null;
			}
			$session = &$_SESSION['_tracy']['bluescreen'][$m[2]];
			if ($session) {
				echo 'PacketeryTracy.BlueScreen.loadAjax(', json_encode($session['content'], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE), ');';
				$session = null;
			}
			return true;
		}
