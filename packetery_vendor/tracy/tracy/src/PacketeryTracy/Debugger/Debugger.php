...
class Debugger
{
	...
	public static function dispatch(): void
	{
		if (self::$productionMode || PHP_SAPI === 'cli') {
			...
			throw new \LogicException(
				...
				. ($file ? "Output started at $file:$line." : 'Try PacketeryTracy\OutputDebugger to find where output started.')
			);
		...
		if (self::$productionMode || connection_aborted()) {
			...
			try {
				...
				if ($file && !headers_sent()) {
					header("X-PacketeryTracy-Error-Log: $file", false);
				}
				...
				echo "$exception\nPacketeryTracy is unable to log error: {$e->getMessage()}\n";
			}
		}
	...
	public static function errorHandler(
		...
		if ($severity === E_RECOVERABLE_ERROR || $severity === E_USER_ERROR) {
			...
			$count = &self::getBar()->getPanel('PacketeryTracy:errors')->data["$file|$line|$message"];
	...
	public static function getBar(): Bar
	{
		if (!self::$bar) {
			self::$bar = new Bar;
			self::$bar->addPanel($info = new DefaultBarPanel('info'), 'PacketeryTracy:info');
			$info->cpuUsage = self::$cpuUsage;
			self::$bar->addPanel(new DefaultBarPanel('errors'), 'PacketeryTracy:errors'); // filled by errorHandler()
		}
		return self::$bar;
	}
	...
	/**
	 * Dumps information about a variable in PacketeryTracy Debug Bar.
	 * @tracySkipLocation
	 * @param  mixed  $var
	...
	public static function barDump($var, string $title = null, array $options = [])
	{
		if (!self::$productionMode) {
			static $panel;
			if (!$panel) {
				self::getBar()->addPanel($panel = new DefaultBarPanel('dumps'), 'PacketeryTracy:dumps');
			}
