...
final class FileUpload
{
	...
	{
		$name = PacketeryNette\Utils\Strings::webalize($this->name, '.', false);
	...
	public function move(string $dest)
	{
		...
		PacketeryNette\Utils\Callback::invokeSafe(
			is_uploaded_file($this->tmpName) ? 'move_uploaded_file' : 'rename',
			[$this->tmpName, $dest],
			function (string $message) use ($dest): void {
				throw new PacketeryNette\InvalidStateException("Unable to move uploaded file '$this->tmpName' to '$dest'. $message");
			}
		);
	...
	/**
	 * Loads an image.
	 * @throws PacketeryNette\Utils\ImageException  If the upload was not successful or is not a valid image
	 */
	public function toImage(): PacketeryNette\Utils\Image
	{
		return PacketeryNette\Utils\Image::fromFile($this->tmpName);
	}
