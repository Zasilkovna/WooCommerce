...
use PacketeryNette;
use PacketeryNette\Security\IIdentity;


/**
 * @deprecated by PacketeryNette\Bridges\SecurityHttp\SessionStorage
 */
class UserStorage implements PacketeryNette\Security\IUserStorage
{
	 ...
	 */
	public function getIdentity(): ?PacketeryNette\Security\IIdentity
	{
		$session = $this->getSessionSection(false);
	...
	public function setExpiration(?string $time, int $flags = 0)
	{
		$section = $this->getSessionSection(true);
		if ($time) {
			$time = PacketeryNette\Utils\DateTime::from($time)->format('U');
	...
	protected function getSessionSection(bool $need): ?SessionSection
	{
		...
		$this->sessionSection = $section = $this->sessionHandler->getSection('PacketeryNette.Http.UserStorage/' . $this->namespace);
