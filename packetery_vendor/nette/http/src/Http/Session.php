...
class Session
{
	...
	private const DEFAULT_FILE_LIFETIME = 3 * PacketeryNette\Utils\DateTime::HOUR;
	...
	/** @var bool  has been session started by PacketeryNette? */
	...
	public function start(): void
	{
		...
		try {
			...
			PacketeryNette\Utils\Callback::invokeSafe('session_start', [['read_and_close' => $this->readAndClose]], function (string $message) use (&$e): void {
				$e = new PacketeryNette\InvalidStateException($message);
			});
		} catch (\Throwable $e) {
		}
	...
	{
		if (!session_status() === PHP_SESSION_ACTIVE) {
			throw new PacketeryNette\InvalidStateException('Session is not started.');
		}
	...
	public function regenerateId(): void
	{
		...
		if (session_status() === PHP_SESSION_ACTIVE) {
			if (headers_sent($file, $line)) {
				throw new PacketeryNette\InvalidStateException('Cannot regenerate session ID after HTTP headers have been sent' . ($file ? " (output started at $file:$line)." : '.'));
			}
	...
	public function setName(string $name)
	{
		if (!preg_match('#[^0-9.][^.]*$#DA', $name)) {
			throw new PacketeryNette\InvalidArgumentException('Session name cannot contain dot.');
		}
	 ...
	 * @return static
	 * @throws PacketeryNette\NotSupportedException
	 * @throws PacketeryNette\InvalidStateException
	 */
	public function setOptions(array $options)
	{
		...
		foreach ($options as $key => $value) {
			...
			if (!isset($allowed["session.$normKey"])) {
				$hint = substr((string) PacketeryNette\Utils\Helpers::getSuggestion(array_keys($allowed), "session.$normKey"), 8);
				if ($key !== $normKey) {
					$hint = preg_replace_callback('#_(.)#', function ($m) { return strtoupper($m[1]); }, $hint); // snake_case -> camelCase
				}
				throw new PacketeryNette\InvalidStateException("Invalid session configuration option '$key'" . ($hint ? ", did you mean '$hint'?" : '.'));
			}
		...
		if (!empty($normalized['read_and_close'])) {
			if (session_status() === PHP_SESSION_ACTIVE) {
				throw new PacketeryNette\InvalidStateException('Cannot configure "read_and_close" for already started session.');
			}
	...
	private function configure(array $config): void
	{
		...
		foreach ($config as $key => $value) {
			if ($value === null || ini_get("session.$key") == $value) { // intentionally ==
				...
				if (session_status() === PHP_SESSION_ACTIVE) {
					throw new PacketeryNette\InvalidStateException("Unable to set 'session.$key' to value '$value' when session has been started" . ($this->started ? '.' : ' by session.auto_start or session_start().'));
				}
				if (isset($special[$key])) {
					...
					throw new PacketeryNette\NotSupportedException("Unable to set 'session.$key' to '$value' because function ini_set() is disabled.");
				}
			}
		}
	...
	public function setExpiration(?string $time)
	{
		if ($time === null) {
			...
			$time = PacketeryNette\Utils\DateTime::from($time)->format('U') - time();
			return $this->setOptions([
				'gc_maxlifetime' => $time,
	...
	{
		if ($this->started) {
			throw new PacketeryNette\InvalidStateException('Unable to set handler when session has been started.');
		}
