...
/**
 * HTTP extension for PacketeryNette DI.
 */
class HttpExtension extends PacketeryNette\DI\CompilerExtension
{
	/** @var bool */
	private $cliMode;
	...
	public function getConfigSchema(): PacketeryNette\Schema\Schema
	{
		return Expect::structure([
			...
			'headers' => Expect::arrayOf('scalar|null')->default([
				'X-Powered-By' => 'PacketeryNette Framework 3',
	...
	public function loadConfiguration()
	{
		...
		$builder->addDefinition($this->prefix('requestFactory'))
			->setFactory(PacketeryNette\Http\RequestFactory::class)
			->addSetup('setProxy', [$config->proxy]);

		$request = $builder->addDefinition($this->prefix('request'))
			->setFactory('@PacketeryNette\Http\RequestFactory::fromGlobals');

		$response = $builder->addDefinition($this->prefix('response'))
			->setFactory(PacketeryNette\Http\Response::class);
	...
	private function sendHeaders()
	{
		...
		foreach (['csp', 'cspReportOnly'] as $key) {
			...
			if (strpos($value, "'nonce'")) {
				...
				$value = PacketeryNette\DI\ContainerBuilder::literal(
