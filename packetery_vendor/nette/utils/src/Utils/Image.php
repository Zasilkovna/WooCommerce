...
class Image
{
	 ...
	 * @throws PacketeryNette\NotSupportedException if gd extension is not loaded
	...
	public static function fromFile(string $file, int &$type = null)
	{
		if (!extension_loaded('gd')) {
			throw new PacketeryNette\NotSupportedException('PHP extension GD is not loaded.');
		}
	...
	public static function fromBlank(int $width, int $height, array $color = null)
	{
		...
		if ($width < 1 || $height < 1) {
			throw new PacketeryNette\InvalidArgumentException('Image width and height must be greater than zero.');
		}
	...
	public static function typeToExtension(int $type): string
	{
		if (!isset(self::FORMATS[$type])) {
			throw new PacketeryNette\InvalidArgumentException("Unsupported image type '$type'.");
		}
	...
	protected function setImageResource($image)
	{
		if (!$image instanceof \GdImage && !(is_resource($image) && get_resource_type($image) === 'gd')) {
			throw new PacketeryNette\InvalidArgumentException('Image is not valid.');
		}
		$this->image = $image;
		return $this;
	}
	...
	public static function calculateSize(
		...
		if ($flags & self::STRETCH) { // non-proportional
			if (!$newWidth || !$newHeight) {
				throw new PacketeryNette\InvalidArgumentException('For stretching must be both width and height specified.');
			}
			...
			if (!$newWidth && !$newHeight) {
				throw new PacketeryNette\InvalidArgumentException('At least width or height must be specified.');
			}
	...
	public function save(string $file, int $quality = null, int $type = null): void
	{
		if ($type === null) {
			...
			if (!isset($extensions[$ext])) {
				throw new PacketeryNette\InvalidArgumentException("Unsupported file extension '$ext'.");
			}
			$type = $extensions[$ext];
		}
	 ...
	 * @return mixed
	 * @throws PacketeryNette\MemberAccessException
	 */
	...
	private static function isPercent(&$num): bool
	{
		...
		throw new PacketeryNette\InvalidArgumentException("Expected dimension in int|string, '$num' given.");
	}
	...
	{
		throw new PacketeryNette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
	}
}
