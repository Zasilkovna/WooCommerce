...
use PacketeryNette;
use PacketeryNette\Schema\Elements\AnyOf;
use PacketeryNette\Schema\Elements\Structure;
use PacketeryNette\Schema\Elements\Type;
		...
		foreach ($ro->getProperties() as $prop) {
			...
			if (!$item) {
				...
				if (PHP_VERSION_ID >= 70400 && !$prop->isInitialized($object)) {
					...
					} elseif ($def === null && !PacketeryNette\Utils\Validators::is(null, $type)) {
						$item->required();
					} else {
						$item->default($def);
					}
