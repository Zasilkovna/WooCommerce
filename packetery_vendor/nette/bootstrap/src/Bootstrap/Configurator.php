...
class Configurator
{
	...
	public $defaultExtensions = [
		'application' => [PacketeryNette\Bridges\ApplicationDI\ApplicationExtension::class, ['%debugMode%', ['%appDir%'], '%tempDir%/cache/nette.application']],
		'cache' => [PacketeryNette\Bridges\CacheDI\CacheExtension::class, ['%tempDir%']],
		'constants' => Extensions\ConstantsExtension::class,
		'database' => [PacketeryNette\Bridges\DatabaseDI\DatabaseExtension::class, ['%debugMode%']],
		'decorator' => PacketeryNette\DI\Extensions\DecoratorExtension::class,
		'di' => [PacketeryNette\DI\Extensions\DIExtension::class, ['%debugMode%']],
		'extensions' => PacketeryNette\DI\Extensions\ExtensionsExtension::class,
		'forms' => PacketeryNette\Bridges\FormsDI\FormsExtension::class,
		'http' => [PacketeryNette\Bridges\HttpDI\HttpExtension::class, ['%consoleMode%']],
		'inject' => PacketeryNette\DI\Extensions\InjectExtension::class,
		'view' => [PacketeryNette\Bridges\ApplicationDI\PacketeryLatteExtension::class, ['%tempDir%/cache/view', '%debugMode%']],
		'mail' => PacketeryNette\Bridges\MailDI\MailExtension::class,
		'php' => Extensions\PhpExtension::class,
		'routing' => [PacketeryNette\Bridges\ApplicationDI\RoutingExtension::class, ['%debugMode%']],
	...
	public function setDebugMode($value)
	{
		if (is_string($value) || is_array($value)) {
			...
			throw new PacketeryNette\InvalidArgumentException(sprintf('Value must be either a string, array, or boolean, %s given.', gettype($value)));
		}
	...
	/**
	 * @throws PacketeryNette\NotSupportedException if RobotLoader is not available
	 */
	public function createRobotLoader(): PacketeryNette\Loaders\RobotLoader
	{
		if (!class_exists(PacketeryNette\Loaders\RobotLoader::class)) {
			throw new PacketeryNette\NotSupportedException('RobotLoader not found, do you have `nette/robot-loader` package installed?');
		}

		$loader = new PacketeryNette\Loaders\RobotLoader;
	...
	public function generateContainer(DI\Compiler $compiler): void
	{
		...
		PacketeryNette\Utils\Arrays::invoke($this->onCompile, $this, $compiler);
	}
	...
	protected function getCacheDirectory(): string
	{
		if (empty($this->staticParameters['tempDir'])) {
			throw new PacketeryNette\InvalidStateException('Set path to temporary directory using setTempDirectory().');
		}
		$dir = $this->staticParameters['tempDir'] . '/cache';
		PacketeryNette\Utils\FileSystem::createDir($dir);
		return $dir;
	}
...
class_exists(PacketeryNette\Configurator::class);
