...
class Rules implements \IteratorAggregate
{
	...
	public function addRule($validator, $errorMessage = null, $arg = null)
	{
		if ($validator === Form::VALID || $validator === ~Form::VALID) {
			throw new PacketeryNette\InvalidArgumentException('You cannot use Form::VALID in the addRule method.');
		}
		$rule = new Rule;
	...
	{
		if ($validator === Form::VALID || $validator === ~Form::VALID) {
			throw new PacketeryNette\InvalidArgumentException('You cannot use Form::VALID in the addCondition method.');
	...
	private function adjustOperation(Rule $rule): void
	{
		...
		if (!is_callable($this->getCallback($rule))) {
			$validator = is_scalar($rule->validator)
				? " '$rule->validator'"
				: '';
			throw new PacketeryNette\InvalidArgumentException("Unknown validator$validator for control '{$rule->control->name}'.");
		}
	}
