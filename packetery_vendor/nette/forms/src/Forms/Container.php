...
use PacketeryNette;
use PacketeryNette\Utils\ArrayHash;
 ...
 */
class Container extends PacketeryNette\ComponentModel\Container implements \ArrayAccess
{
	use PacketeryNette\ComponentModel\ArrayAccess;
	...
	public function setValues($data, bool $erase = false)
	{
		if ($data instanceof \Traversable) {
			...
			throw new PacketeryNette\InvalidArgumentException(sprintf('First parameter must be an array or object, %s given.', gettype($data)));
		}
		...
		foreach ($this->getComponents() as $name => $control) {
			...
			if (
				...
				$type = $returnType === self::ARRAY && !$control->mappedType
					? self::ARRAY
					: ($rc->hasProperty($name) ? PacketeryNette\Utils\Reflection::getPropertyType($rc->getProperty($name)) : null);
	 ...
	 */
	public function addComponent(PacketeryNette\ComponentModel\IComponent $component, ?string $name, string $insertBefore = null)
	{
	...
	{
		throw new PacketeryNette\NotImplementedException('Form cloning is not supported yet.');
	}
}
