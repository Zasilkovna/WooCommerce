...
class Validator
{
	...
	/**
	 * @return string|PacketeryNette\HtmlStringable
	 * @internal
	 */
	public static function formatMessage(Rule $rule, bool $withValue = true)
	{
		$message = $rule->message;
		if ($message instanceof PacketeryNette\HtmlStringable) {
			return $message;
			...
			switch ($m[1]) {
				...
				case 'label':
					if ($rule->control instanceof Controls\BaseControl) {
						...
						$caption = $caption instanceof PacketeryNette\HtmlStringable
							? $caption->getText()
	...
	public static function validatePattern(Control $control, string $pattern, bool $caseInsensitive = false): bool
	{
		...
		foreach (static::toArray($control->getValue()) as $item) {
			$value = $item instanceof PacketeryNette\Http\FileUpload ? $item->getName() : $item;
