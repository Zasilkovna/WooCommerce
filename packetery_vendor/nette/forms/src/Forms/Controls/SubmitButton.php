 ...
 */
class SubmitButton extends Button implements PacketeryNette\Forms\SubmitterControl
{
	/**
	 * Occurs when the button is clicked and form is successfully validated
	 * @var array<callable(self, array|object): void|callable(PacketeryNette\Forms\Form, array|object): void|callable(array|object): void>
	 */
	public $onClick = [];
	...
	public function setValidationScope(?iterable $scope)
	{
		if ($scope === null) {
			...
			foreach ($scope ?: [] as $control) {
				if (!$control instanceof PacketeryNette\Forms\Container && !$control instanceof PacketeryNette\Forms\Control) {
					throw new PacketeryNette\InvalidArgumentException('Validation scope accepts only PacketeryNette\Forms\Container or PacketeryNette\Forms\IControl instances.');
				}
	...
	{
		PacketeryNette\Utils\Arrays::invoke($this->onClick, $this);
	}


	public function getControl($caption = null): PacketeryNette\Utils\Html
	{
		$scope = [];
		foreach ((array) $this->validationScope as $control) {
			$scope[] = $control->lookupPath(PacketeryNette\Forms\Form::class);
		}
