...
use PacketeryNette;
use PacketeryNette\Forms\Control;
use PacketeryNette\Forms\Form;
use PacketeryNette\Forms\Rules;
 ...
 */
abstract class BaseControl extends PacketeryNette\ComponentModel\Component implements Control
{
	/** @var string */
	...
	/** @var PacketeryNette\Localization\Translator|bool|null */
	...
	{
		return $this->control->name ?? PacketeryNette\Forms\Helpers::generateHtmlName($this->lookupPath(Form::class));
	}
	...
	public function getControl()
	{
		...
		return $el->addAttributes([
			...
			'data-nette-rules' => PacketeryNette\Forms\Helpers::exportRules($this->rules) ?: null,
		]);
	}
	...
	public function getLabel($caption = null)
	{
		...
		$label->setText($translator && !$caption instanceof PacketeryNette\HtmlStringable ? $translator->translate($caption) : $caption);
		return $label;
	}
	...
	public function getHtmlId()
	{
		if (!isset($this->control->id)) {
			$form = $this->getForm();
			$prefix = $form instanceof PacketeryNette\Application\UI\Form || $form->getName() === null
				? ''
				: $form->getName() . '-';
	...
	public function translate($value, ...$parameters)
	{
		if ($translator = $this->getTranslator()) {
			...
			foreach ($tmp[0] as &$v) {
				if ($v != null && !$v instanceof PacketeryNette\HtmlStringable) { // intentionally ==
