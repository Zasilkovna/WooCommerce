...
final class Helpers
{
	...
	public static function expand($var, array $params, $recursive = false)
	{
		...
		foreach ($parts as $n => $part) {
			if ($n % 2 === 0) {
				...
				throw new PacketeryNette\InvalidArgumentException(sprintf('Circular reference detected for variables: %s.', implode(', ', array_keys($recursive))));
				...
				foreach (explode('.', $part) as $key) {
					if (is_array($val) && array_key_exists($key, $val)) {
						...
						throw new PacketeryNette\InvalidArgumentException("Missing parameter '$part'.");
					}
				}
				if ($recursive) {
				...
				if ($val instanceof DynamicParameter) {
					...
					throw new PacketeryNette\InvalidArgumentException("Unable to concatenate non-scalar parameter '$part' into '$var'.");
				}
				$res[] = $val;
			}
		}
	...
	public static function parseAnnotation(\Reflector $ref, string $name): ?string
	{
		if (!Reflection::areCommentsAvailable()) {
			throw new PacketeryNette\InvalidStateException('You have to enable phpDoc comments in opcode cache.');
		}
	...
	public static function convertType($value, string $type)
	{
		...
		throw new PacketeryNette\InvalidStateException("Cannot convert $value to $type.");
	}
}
