...
class Loader
{
	...
	public function load(string $file, ?bool $merge = true): array
	{
		if (!is_file($file) || !is_readable($file)) {
			throw new PacketeryNette\FileNotFoundException("File '$file' is missing or is not readable.");
		}

		if (isset($this->loadedFiles[$file])) {
			throw new PacketeryNette\InvalidStateException("Recursive included file '$file'");
		}
		...
		if (isset($data[self::INCLUDES_KEY])) {
			...
			$includes = PacketeryNette\DI\Helpers::expand($data[self::INCLUDES_KEY], $this->parameters);
			foreach ($includes as $include) {
				$include = $this->expandIncludedFile($include, $file);
				$res = PacketeryNette\Schema\Helpers::merge($this->load($include, $merge), $res);
			}
		}
		...
		if ($merge === false) {
			...
			$res = PacketeryNette\Schema\Helpers::merge($data, $res);
		}
		return $res;
	}
	...
	public function save(array $data, string $file): void
	{
		if (file_put_contents($file, $this->getAdapter($file)->dump($data)) === false) {
			throw new PacketeryNette\IOException("Cannot write file '$file'.");
		}
	}
	...
	private function getAdapter(string $file): Adapter
	{
		...
		if (!isset($this->adapters[$extension])) {
			throw new PacketeryNette\InvalidArgumentException("Unknown file extension '$file'.");
		}
