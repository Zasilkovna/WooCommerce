...
final class LocatorDefinition extends Definition
{
	...
	public function setImplement(string $type)
	{
		if (!interface_exists($type)) {
			throw new PacketeryNette\InvalidArgumentException(sprintf("Service '%s': Interface '%s' not found.", $this->getName(), $type));
		}
		$methods = (new \ReflectionClass($type))->getMethods();
		if (!$methods) {
			throw new PacketeryNette\InvalidArgumentException(sprintf("Service '%s': Interface %s must have at least one method.", $this->getName(), $type));
		}
		...
		foreach ($methods as $method) {
			if ($method->isStatic() || !(
				...
				throw new PacketeryNette\InvalidArgumentException(sprintf(
	...
	' . ($nullable ? 'return null;' : 'throw new PacketeryNette\DI\MissingServiceException("Service \'$name\' is not defined.");') . '
}
return $this->container->' . $m[1] . 'Service($this->mapping[$name]);')
				...
				if ($m[1] === 'get') {
					...
					$methodInner->setBody('return $this->container->?();', [PacketeryNette\DI\Container::getMethodName($ref)]);
				}
			} else {
				$methodInner->setBody($nullable ? 'return null;' : 'throw new PacketeryNette\DI\MissingServiceException("Service is not defined.");');
			}
		}
