...
use PacketeryNette;
use PacketeryNette\DI\ServiceCreationException;
use PacketeryNette\PhpGenerator\Helpers as PhpHelpers;
...
final class ServiceDefinition extends Definition
{
	...
	public function setSetup(array $setup)
	{
		foreach ($setup as $v) {
			if (!$v instanceof Statement) {
				throw new PacketeryNette\InvalidArgumentException('Argument must be PacketeryNette\DI\Definitions\Statement[].');
			}
		}
		$this->setup = $setup;
		return $this;
	...
	{
		throw new PacketeryNette\DeprecatedException(sprintf('Service %s: %s() is deprecated.', $this->getName(), __METHOD__));
	}
	...
	{
		throw new PacketeryNette\DeprecatedException(sprintf('Service %s: %s() is deprecated, use $builder->addImportedDefinition(...)', $this->getName(), __METHOD__));
	}
	...
	{
		throw new PacketeryNette\DeprecatedException(sprintf('Service %s: %s() is deprecated, use $builder->addFactoryDefinition(...)', $this->getName(), __METHOD__));
	}
	...
	{
		trigger_error(sprintf('Service %s: %s() is deprecated, use addTag(PacketeryNette\DI\Extensions\InjectExtension::TAG_INJECT)', $this->getName(), __METHOD__), E_USER_DEPRECATED);
		return $this->addTag(PacketeryNette\DI\Extensions\InjectExtension::TAG_INJECT, $state);
	}
	...
	public function complete(PacketeryNette\DI\Resolver $resolver): void
	{
		...
		if ($entity instanceof Reference && !$this->factory->arguments && !$this->setup) {
			...
			$this->setFactory([new Reference(PacketeryNette\DI\ContainerBuilder::THIS_CONTAINER), 'getService'], [$ref->getValue()]);
		}
	...
	public function generateMethod(PacketeryNette\PhpGenerator\Method $method, PacketeryNette\DI\PhpGenerator $generator): void
	{
		$entity = $this->factory->getEntity();
		...
		if (
			$type !== $entity
			&& !(is_array($entity) && $entity[0] instanceof Reference && $entity[0]->getValue() === PacketeryNette\DI\ContainerBuilder::THIS_CONTAINER)
			&& !(is_string($entity) && preg_match('#^[\w\\\\]+$#D', $entity) && is_subclass_of($entity, $type))
		) {
			$code .= PhpHelpers::formatArgs(
				"if (!\$service instanceof $type) {\n"
				. "\tthrow new PacketeryNette\\UnexpectedValueException(?);\n}\n",
...
class_exists(PacketeryNette\DI\ServiceDefinition::class);
