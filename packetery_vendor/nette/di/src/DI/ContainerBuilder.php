...
use PacketeryNette;
use PacketeryNette\DI\Definitions\Definition;
...
class ContainerBuilder
{
	...
	public function addDefinition(?string $name, Definition $definition = null): Definition
	{
		...
		if ($name === null) {
			...
			throw new PacketeryNette\InvalidArgumentException(sprintf('Service name must be a alpha-numeric string and not a number, %s given.', gettype($name)));

		} else {
			$name = $this->aliases[$name] ?? $name;
			if (isset($this->definitions[$name])) {
				throw new PacketeryNette\InvalidStateException("Service '$name' has already been added.");
			}
			$lname = strtolower($name);
			foreach ($this->definitions as $nm => $foo) {
				if ($lname === strtolower($nm)) {
					throw new PacketeryNette\InvalidStateException("Service '$name' has the same name as '$nm' in a case-insensitive manner.");
				}
			}
		}
	...
	public function addAlias(string $alias, string $service): void
	{
		if (!$alias) { // builder is not ready for falsy names such as '0'
			throw new PacketeryNette\InvalidArgumentException(sprintf('Alias name must be a non-empty string, %s given.', gettype($alias)));

		} elseif (!$service) { // builder is not ready for falsy names such as '0'
			throw new PacketeryNette\InvalidArgumentException(sprintf('Service name must be a non-empty string, %s given.', gettype($service)));

		} elseif (isset($this->aliases[$alias])) {
			throw new PacketeryNette\InvalidStateException("Alias '$alias' has already been added.");

		} elseif (isset($this->definitions[$alias])) {
			throw new PacketeryNette\InvalidStateException("Service '$alias' has already been added.");
		}
	...
	public static function literal(string $code, array $args = null): PacketeryNette\PhpGenerator\PhpLiteral
	{
		return new PacketeryNette\PhpGenerator\PhpLiteral(
			$args === null ? $code : PacketeryNette\PhpGenerator\Helpers::formatArgs($code, $args)
		);
	}
