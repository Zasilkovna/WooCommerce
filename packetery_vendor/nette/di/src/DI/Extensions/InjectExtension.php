...
final class InjectExtension extends DI\CompilerExtension
{
	...
	{
		return PacketeryNette\Schema\Expect::structure([]);
	}
			...
			if ($hasAttr || DI\Helpers::parseAnnotation($rp, 'inject') !== null) {
				if ($type = Reflection::getPropertyType($rp)) {
					...
					if (strpos($type, '|') !== false) {
						throw new PacketeryNette\InvalidStateException('The ' . Reflection::toString($rp) . ' is not expected to have a union type.');
					}
	...
	public static function callInjects(DI\Container $container, $service): void
	{
		if (!is_object($service)) {
			throw new PacketeryNette\InvalidArgumentException(sprintf('Service must be object, %s given.', gettype($service)));
		}
	...
	private static function checkType($class, string $name, ?string $type, $container): void
	{
		...
		if (!$type) {
			throw new PacketeryNette\InvalidStateException("Property $propName has no type hint.");
		} elseif (!class_exists($type) && !interface_exists($type)) {
			throw new PacketeryNette\InvalidStateException("Class or interface '$type' used in type hint at $propName not found. Check type and 'use' statements.");
		} elseif ($container && !$container->getByType($type, false)) {
			throw new PacketeryNette\DI\MissingServiceException("Service of type $type used in type hint at $propName not found. Did you add it to configuration file?");
		}
	}
}
