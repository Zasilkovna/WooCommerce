...
class Container
{
	...
	public function addService(string $name, $service)
	{
		...
		if (isset($this->instances[$name])) {
			throw new PacketeryNette\InvalidStateException("Service '$name' already exists.");

		} elseif (!is_object($service)) {
			throw new PacketeryNette\InvalidArgumentException(sprintf("Service '%s' must be a object, %s given.", $name, gettype($service)));
		}

		$type = $service instanceof \Closure
			? (string) PacketeryNette\Utils\Reflection::getReturnType(new \ReflectionFunction($service))
			: get_class($service);
		...
		if (!isset($this->methods[self::getMethodName($name)])) {
			...
			throw new PacketeryNette\InvalidArgumentException("Service '$name' must be instance of $expectedType, " . ($type ? "$type given." : 'add typehint to closure.'));
		}
	...
	public function createService(string $name, array $args = [])
	{
		...
		if (isset($this->creating[$name])) {
			throw new PacketeryNette\InvalidStateException(sprintf('Circular reference detected for services: %s.', implode(', ', array_keys($this->creating))));
		...
		if (!is_object($service)) {
			throw new PacketeryNette\UnexpectedValueException("Unable to create service '$name', value returned by " . ($cb instanceof \Closure ? 'closure' : "method $method()") . ' is not object.');
		}
	...
	{
		return $function(...$this->autowireArguments(PacketeryNette\Utils\Callback::toReflection($function), $args));
	}
	...
	{
		if ($name === '') {
			throw new PacketeryNette\InvalidArgumentException('Service name must be a non-empty string.');
		}
