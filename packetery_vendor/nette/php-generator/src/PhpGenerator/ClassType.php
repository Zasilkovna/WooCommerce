...
final class ClassType
{
	...
	public function setName(?string $name): self
	{
		if ($name !== null && !Helpers::isIdentifier($name)) {
			throw new PacketeryNette\InvalidArgumentException("Value '$name' is not valid class name.");
		}
		$this->name = $name;
		return $this;
	}
	...
	public function setType(string $type): self
	{
		if (!in_array($type, [self::TYPE_CLASS, self::TYPE_INTERFACE, self::TYPE_TRAIT], true)) {
			throw new PacketeryNette\InvalidArgumentException('Argument must be class|interface|trait.');
		}
		$this->type = $type;
		return $this;
	}
	...
	public function setExtends($names): self
	{
		if (!is_string($names) && !is_array($names)) {
			throw new PacketeryNette\InvalidArgumentException('Argument must be string or string[].');
		}
	...
	public function addMember($member): self
	{
		if ($member instanceof Method) {
			...
			throw new PacketeryNette\InvalidArgumentException('Argument must be Method|Property|Constant.');
		}
	...
	public function setProperties(array $props): self
	{
		...
		foreach ($props as $v) {
			if (!$v instanceof Property) {
				throw new PacketeryNette\InvalidArgumentException('Argument must be PacketeryNette\PhpGenerator\Property[].');
			}
	...
	public function getProperty(string $name): Property
	{
		if (!isset($this->properties[$name])) {
			throw new PacketeryNette\InvalidArgumentException("Property '$name' not found.");
		}
	...
	public function setMethods(array $methods): self
	{
		...
		foreach ($methods as $v) {
			if (!$v instanceof Method) {
				throw new PacketeryNette\InvalidArgumentException('Argument must be PacketeryNette\PhpGenerator\Method[].');
			}
	...
	public function getMethod(string $name): Method
	{
		if (!isset($this->methods[$name])) {
			throw new PacketeryNette\InvalidArgumentException("Method '$name' not found.");
		}
	...
	public function validate(): void
	{
		if ($this->abstract && $this->final) {
			throw new PacketeryNette\InvalidStateException('Class cannot be abstract and final.');

		} elseif (!$this->name && ($this->abstract || $this->final)) {
			throw new PacketeryNette\InvalidStateException('Anonymous class cannot be abstract or final.');
		}
	}
