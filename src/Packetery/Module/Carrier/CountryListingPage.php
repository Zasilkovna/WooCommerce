...
class CountryListingPage {
	...
	public const DATA_KEY_CODE             = 'code';
	...
	private function getActiveCountries(): array {
		...
		foreach ( $countries as $country ) {
			...
			$countriesFinal[] = [
				self::DATA_KEY_CODE => $country,
				'name'              => $wcCountries[ strtoupper( $country ) ],
				'url'               => add_query_arg(
					[
						'page'                           => OptionsPage::SLUG,
						OptionsPage::PARAMETER_NAME_CODE => $country,
					],
					get_admin_url( null, 'admin.php' )
				),
		...
		usort(
			$countriesFinal,
			static function ( $a, $b ) {
				if ( 'cz' === $a[ self::DATA_KEY_CODE ] ) {
					return - 1;
				}
				if ( 'cz' === $b[ self::DATA_KEY_CODE ] ) {
					return 1;
				}
				if ( 'sk' === $a[ self::DATA_KEY_CODE ] ) {
					return - 1;
				}
				if ( 'sk' === $b[ self::DATA_KEY_CODE ] ) {
					return 1;
				}
