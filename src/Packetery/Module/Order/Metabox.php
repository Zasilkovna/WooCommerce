...
class Metabox {
	...
	private function prepareMetaboxParts(): array {
		...
		$this->orderForm->setDefaults(
			...
			$order->getFinalWeight(),
			$unit === OptionsProvider::DIMENSIONS_UNIT_CM ? CoreHelper::convertToCentimeters( (int) $order->getLength(), 'mm' ) : $order->getLength(),
			$unit === OptionsProvider::DIMENSIONS_UNIT_CM ? CoreHelper::convertToCentimeters( (int) $order->getWidth(), 'mm' ) : $order->getWidth(),
			$unit === OptionsProvider::DIMENSIONS_UNIT_CM ? CoreHelper::convertToCentimeters( (int) $order->getHeight(), 'mm' ) : $order->getHeight(),
			$order->getCod(),
			$order->getValue(),
