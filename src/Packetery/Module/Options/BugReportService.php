...
class BugReportService {
	...
	public function createForm(): Form {
		...
		$form->addEmail( 'replyTo', $this->wpAdapter->__( 'Reply email', 'packeta' ) )
	 ...
	 * @param array|object $values Form values.
	 * @phpstan-param array<string, mixed>|object $values
	 */
	...
	public function onFormValidate( Container $form ): void {
		$valuesArray = (array) $form->getValues();
	...
	private function sendBugReport( string $replyEmail, string $message ): bool {
		...
		$subject  = sprintf( 'Packeta: Plugin WP - hlášení problémů - %s', $siteName );

		/**
		 * @var array $headers
		 * @phpstan-var array{0:string,1:string,2:string} $headers
		 */
		$headers = [
		...
		return $this->latteEngine->renderToString(
			...
			[
				...
				'translations' => [
					...
					'replyEmail'     => $this->wpAdapter->__( 'Reply Email:', 'packeta' ),
	...
	private function createAttachments(): array {
		...
		if ( $zip->open( $zipPath, ZipArchive::CREATE ) === true ) {
			$this->addPluginSettingsToZip( $zip );
	...
	private function addPluginSettingsToZip( ZipArchive $zip ): void {
	...
	private function addWooCommerceSystemStatusToZip( ZipArchive $zip ): void {
		...
		ob_start();
		$this->wcAdapter->getSystemStatusReport();
